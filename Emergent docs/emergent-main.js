  ...
  function detectBuildError() {
    ...
    if (!errorDetails) {
      const webpackOverlay =
        document.querySelector('iframe[id*="webpack"]') ||
      ...
      if (webpackOverlay) {
        try {
          ...
          if (webpackOverlay.tagName === 'IFRAME') {
            const iframeDoc =
              webpackOverlay.contentDocument || webpackOverlay.contentWindow?.document;
            if (iframeDoc) {
          ...
          errorDetails = {
            message:
              'Webpack build error detected (unable to extract details due to iframe restrictions)',
            tool: 'Webpack',
            error: e.message,
          };
        }
    ...
    if (!errorDetails) {
      ...
      if (hasError) {
        ...
        const errorDiv =
          document.querySelector('[class*="error"]') || document.querySelector('[id*="error"]');
    ...
    const reactError =
      document.querySelector('[data-test-id="error-boundary"]') ||

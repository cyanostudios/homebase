// plugins/instructions/controller.js
...
class InstructionController {
  ...
  async getAll(req, res) {
    try {
      ...
      Logger.error('Get instructions failed', error, { userId: Context.getUserId(req) });
  ...
  async getById(req, res) {
    try {
      ...
      if (!item) {
        return res.status(404).json({ error: 'Instruction not found', code: 'NOT_FOUND' });
      }
      res.json(item);
    } catch (error) {
      Logger.error('Get instruction failed', error, {
  ...
  async create(req, res) {
    try {
      ...
      Logger.error('Create instruction failed', error, { userId: Context.getUserId(req) });
  ...
  async update(req, res) {
    try {
      ...
      Logger.error('Update instruction failed', error, {
        instructionId: req.params.id,
        userId: Context.getUserId(req),
      });
      const mapped = this.mapUniqueViolation(error);
  ...
  async reorder(req, res) {
    try {
      ...
      Logger.error('Reorder instructions failed', error, { userId: Context.getUserId(req) });
      if (error instanceof AppError) {
        return this.sendAppError(res, error);
      }
  ...
  async listCategories(req, res) {
    try {
      ...
      Logger.error('List instruction categories failed', error, {
        userId: Context.getUserId(req),
      });
      if (error instanceof AppError) {
        return res.status(error.statusCode).json(error.toJSON());
  ...
  async createCategory(req, res) {
    try {
      ...
      Logger.error('Create instruction category failed', error, {
        userId: Context.getUserId(req),
      });
      const mapped = this.mapUniqueViolation(error);
      if (mapped) {
  ...
  async reorderCategories(req, res) {
    try {
      ...
      Logger.error('Reorder instruction categories failed', error, {
        userId: Context.getUserId(req),
      });
      if (error instanceof AppError) {
        return this.sendAppError(res, error);
  ...
  async delete(req, res) {
    try {
      ...
      req.activityLogEntityName = result.backup?.title || 'Unknown Instruction';
      ...
      Logger.error('Delete instruction failed', error, {
        instructionId: req.params.id,
        userId: Context.getUserId(req),
      });
      if (error instanceof AppError) {
  ...
  async bulkDelete(req, res) {
    try {
      ...
      req.activityLogEntityName = `${deleted} instructions`;
      req.activityLogMetadata = {
        count: deleted,
      ...
      Logger.error('Bulk delete instructions failed', error, {
        userId: Context.getUserId(req),
      });
      if (error instanceof AppError) {
        return res.status(error.statusCode).json(error.toJSON());

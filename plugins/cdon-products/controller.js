...
class CdonProductsController {
  ...
  async bulkCreateArticles(req, res) {
    try {
      ...
      return res.status(502).json({ ok: false, error: 'CDON request failed', detail: error?.message });
    }
  }
  ...
  async exportProducts(req, res) {
    try {
      ...
      for (const p of normalized) {
        ...
        const pricing = raw?.channelSpecific?.pricing?.markets;
        ...
        if (article) {
          ...
          items.push({ productId: p.productId, status: 'error', error: 'Mapper returned null (check required fields)' });
        }
      }
      ...
      return res.json({
        ok: true,
        endpoint: url,
        counts: { requested: rawProducts.length, success: articlesMeta.length, error: items.filter((x) => x.status === 'error').length },
        items,
      });
    } catch (error) {

...
class InspectionModel {
  ...
  async getProjectFileIds(req, projectId) {
    const db = Database.get(req);
    const userId = typeof Context?.getUserId === 'function' ? Context.getUserId(req) : null;

    // Först: försök med user_id om kolumnen finns, annars fall back.
    try {
      if (userId != null) {
        const rows = await db.query(
          `SELECT pf.file_id
  ...
  async getProjectFiles(req, projectId) {
    ...
    // Målet här är att undvika N+1. Vi hämtar länkning + filmetadata i samma query.
    // Vi försöker först filtrera på pf.user_id om kolumnen finns, annars fall back.
    ...
    let rows = [];
    try {
      rows = await tryWithUserId();
    } catch (e) {
      rows = await tryWithoutUserId();
    }

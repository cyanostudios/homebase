...
class InspectionModel {
  ...
  async addFiles(req, projectId, fileIds) {
    try {
      ...
      const existingFileIds = new Set((existing.files || []).map((f) => String(f.id)));
      const toAdd = ids.filter((id) => id && !existingFileIds.has(String(id)));
  ...
  /** Ersätter projektets filer med den angivna listan. */
  async setFiles(req, projectId, fileIds) {
    try {
      ...
      Logger.info('Files set for inspection project', { projectId, count: ids.length });
      return this.getById(req, projectId);
    } catch (error) {
      if (error instanceof AppError) throw error;

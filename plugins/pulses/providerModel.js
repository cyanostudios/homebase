...
function normalizeProviderKey(value) {
  ...
  if (!SUPPORTED_PROVIDERS.has(normalized)) {
    throw new AppError('Unsupported Pulse provider', 400, AppError.CODES.VALIDATION_ERROR);
  }
  return normalized;
}
...
class PulseProviderSettingsModel {
  _requireUserId(req) {
  ...
  _transformRow(row, { includeSecret = false } = {}) {
    ...
    const out = {
      ...
      smsNotificationCapable: isSmsNotificationCapable(row.provider_key),
      verifyCapable: getProviderCatalogEntry(row.provider_key)?.verifyCapable === true,
    ...
    try {
      ...
      if (!row) {
        return {
          ...
          options: {},
          configured: normalizedProvider === 'mock',
          smsNotificationCapable: isSmsNotificationCapable(normalizedProvider),
          verifyCapable: getProviderCatalogEntry(normalizedProvider)?.verifyCapable === true,
          createdAt: null,
          updatedAt: null,
        };
      }
      ...
      Logger.error('Failed to fetch Pulse provider settings', error);
      if (error instanceof AppError) throw error;
      throw new AppError(
        'Failed to fetch Pulse provider settings',
        500,
  ...
  async listConfiguredSettings(req) {
    try {
      ...
      Logger.error('Failed to list Pulse provider settings', error);
      if (error instanceof AppError) throw error;
      throw new AppError(
        'Failed to list Pulse provider settings',
        500,
        AppError.CODES.DATABASE_ERROR,
      );
    }
  ...
  async deleteSettings(req, providerKey) {
    try {
      ...
      const userId = this._requireUserId(req);
      const normalizedProvider = normalizeProviderKey(providerKey);
      // Clear routing assignments that would orphan SMS send after credential delete.
      ...
      Logger.error('Failed to delete Pulse provider settings', error);
      if (error instanceof AppError) throw error;
      throw new AppError(
        'Failed to delete Pulse provider settings',
        500,
        AppError.CODES.DATABASE_ERROR,
      );
    }
  ...
  async saveSettings(req, providerKey, data) {
    try {
      ...
      Logger.error('Failed to save Pulse provider settings', error);
      if (error instanceof AppError) throw error;
      throw new AppError(
        'Failed to save Pulse provider settings',
        500,
        AppError.CODES.DATABASE_ERROR,
      );
    }
  ...
  async getPreferredEnabledSmsProviderKey(req) {
    for (const key of Object.keys(PROVIDER_CATALOG)) {
      if (!isSmsNotificationCapable(key)) continue;
  ...
  async _assertRoutingProviderAvailable(req, providerKey) {
    const normalized = normalizeProviderKey(providerKey);
    if (!isSmsNotificationCapable(normalized)) {
      throw new AppError(
        'Selected provider cannot send SMS notifications',
        400,
        'provider_not_sms_capable',
      );
    }
  ...
  async getRoutingForScope(req, scope) {
    try {
      ...
      Logger.error('Failed to fetch Pulse provider routing', error);
      if (error instanceof AppError) throw error;
      throw new AppError(
        'Failed to fetch Pulse provider routing',
        500,
        AppError.CODES.DATABASE_ERROR,
      );
    }
  ...
  async listRouting(req) {
    try {
      ...
      if (error instanceof AppError) throw error;
      throw new AppError(
        'Failed to list Pulse provider routing',
        500,
        AppError.CODES.DATABASE_ERROR,
      );
    }
  ...
  async saveRouting(req, scope, data) {
    try {
      ...
      Logger.error('Failed to save Pulse provider routing', error);
      if (error instanceof AppError) throw error;
      throw new AppError(
        'Failed to save Pulse provider routing',
        500,
        AppError.CODES.DATABASE_ERROR,
      );
    }
  ...
  async deletePluginRouting(req, pluginKey) {
    try {
      ...
      if (error instanceof AppError) throw error;
      throw new AppError(
        'Failed to delete Pulse provider routing override',
        500,
        AppError.CODES.DATABASE_ERROR,
      );
    }
...
const providerModel = new PulseProviderSettingsModel();

module.exports = providerModel;
module.exports.PulseProviderSettingsModel = PulseProviderSettingsModel;

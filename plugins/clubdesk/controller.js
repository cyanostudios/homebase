// plugins/clubdesk/controller.js
const { Logger, Context } = require('@homebase/core');
const { AppError } = require('../../server/core/errors/AppError');

class ClubdeskController {
  constructor(model) {
    this.model = model;
  }
  ...
  async getAll(req, res) {
    try {
      ...
      Logger.error('Get clubdesk failed', error, { userId: Context.getUserId(req) });
      if (error instanceof AppError) {
        return res.status(error.statusCode).json(error.toJSON());
      }
      res.status(500).json({ error: 'Failed to fetch clubdesk' });
    }
  }
  ...
  async getById(req, res) {
    try {
      ...
      if (!item) {
        return res.status(404).json({ error: 'Clubdesk not found', code: 'NOT_FOUND' });
      }
      res.json(item);
    } catch (error) {
      Logger.error('Get clubdesk failed', error, {
        clubdeskId: req.params.id,
  ...
  async create(req, res) {
    try {
      ...
      Logger.error('Create clubdesk failed', error, { userId: Context.getUserId(req) });
      ...
      res.status(500).json({ error: 'Failed to create clubdesk' });
    }
  }
  ...
  async update(req, res) {
    try {
      ...
      Logger.error('Update clubdesk failed', error, {
        clubdeskId: req.params.id,
        userId: Context.getUserId(req),
      });
      const mapped = this.mapUniqueViolation(error);
      ...
      res.status(500).json({ error: 'Failed to update clubdesk' });
    }
  }
  ...
  async reorder(req, res) {
    try {
      ...
      const items = await this.model.reorderInCategory(req, category, orderedIds);
      res.json(items);
    } catch (error) {
      Logger.error('Reorder clubdesk failed', error, { userId: Context.getUserId(req) });
      if (error instanceof AppError) {
        return this.sendAppError(res, error);
      }
      res.status(500).json({ error: 'Failed to reorder clubdesk' });
    }
  }

  async listCategories(req, res) {
    try {
      const items = await this.model.listCategories(req);
      res.json(items);
    } catch (error) {
      Logger.error('List clubdesk categories failed', error, {
        userId: Context.getUserId(req),
      });
      if (error instanceof AppError) {
        return res.status(error.statusCode).json(error.toJSON());
  ...
  async createCategory(req, res) {
    try {
      const item = await this.model.createCategory(req, req.body);
      res.json(item);
    } catch (error) {
      Logger.error('Create clubdesk category failed', error, {
        userId: Context.getUserId(req),
      });
      const mapped = this.mapUniqueViolation(error);
      if (mapped) {
  ...
  async reorderCategories(req, res) {
    try {
      const items = await this.model.reorderCategories(req, req.body?.orderedIds);
      res.json(items);
    } catch (error) {
      Logger.error('Reorder clubdesk categories failed', error, {
        userId: Context.getUserId(req),
      });
      if (error instanceof AppError) {
        return this.sendAppError(res, error);
  ...
  async deleteCategory(req, res) {
    try {
      ...
      Logger.error('Delete clubdesk category failed', error, {
  ...
  async delete(req, res) {
    try {
      ...
      req.activityLogEntityName = result.backup?.title || 'Unknown Clubdesk';
      req.activityLogMetadata = {
        backup: result.backup,
      };

      res.json({ message: 'Clubdesk deleted successfully' });
    } catch (error) {
      Logger.error('Delete clubdesk failed', error, {
        clubdeskId: req.params.id,
        userId: Context.getUserId(req),
      });
      if (error instanceof AppError) {
      ...
      res.status(500).json({ error: 'Failed to delete clubdesk' });
    }
  }
  ...
  async bulkDelete(req, res) {
    try {
      ...
      req.activityLogEntityName = `${deleted} clubdesk`;
      req.activityLogMetadata = {
        count: deleted,
      ...
      Logger.error('Bulk delete clubdesk failed', error, {
        userId: Context.getUserId(req),
      });
      if (error instanceof AppError) {
        return res.status(error.statusCode).json(error.toJSON());
...
module.exports = ClubdeskController;

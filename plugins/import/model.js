class ImportModel {
  ...
  async logImportOperation(userId, operation) {
    const query = `
  ...
  validateCsvStructure(csvData, pluginType) {
    const validationRules = {
      contacts: {
        ...
        transform: (row) => ({
          companyName: row.companyname?.trim() || row.companyName?.trim(),
          contactType: row.contacttype?.trim().toLowerCase() || row.contactType?.trim().toLowerCase(),
          email: row.email?.trim().toLowerCase(),
          phone: row.phone?.trim() || '',
          phone2: row.phone2?.trim() || '',
          organizationNumber: row.organizationnumber?.trim() || row.organizationNumber?.trim() || '',
          personalNumber: row.personalnumber?.trim() || row.personalNumber?.trim() || '',
          website: row.website?.trim() || '',
          notes: row.notes?.trim() || '',
          companyType: row.companytype?.trim() || row.companyType?.trim() || '',
          vatNumber: row.vatnumber?.trim() || row.vatNumber?.trim() || '',
          taxRate: row.taxrate?.trim() || row.taxRate?.trim() || '',
          paymentTerms: row.paymentterms?.trim() || row.paymentTerms?.trim() || '',
          currency: row.currency?.trim() || 'SEK',
          fTax: row.ftax?.trim() || row.fTax?.trim() || '',

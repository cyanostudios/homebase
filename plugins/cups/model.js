...
class CupsModel {
  ...
  isOnlyLocationDifference(existingRow, payload) {
    ...
    return (
      ...
      (!Object.prototype.hasOwnProperty.call(payload, 'featured_image_drive_url') ||
        sameString('featured_image_drive_url')) &&
  ...
  transformRow(row) {
    ...
    return {
      ...
      featured_image_drive_url: row.featured_image_drive_url ?? null,
    ...
    try {
      ...
      const row = await db.insert('cups', {
        ...
        featured_image_drive_url:
          data.featured_image_drive_url != null &&
          String(data.featured_image_drive_url).trim() !== ''
            ? String(data.featured_image_drive_url).trim()
            : null,
        featured_image_url:
  ...
  async update(req, id, data) {
    try {
      ...
      const row = await db.update('cups', id, {
        ...
        registration_url: data.registration_url?.trim() || null,
        featured_image_drive_url:
          data.featured_image_drive_url !== undefined
            ? data.featured_image_drive_url != null &&
              String(data.featured_image_drive_url).trim() !== ''
              ? String(data.featured_image_drive_url).trim()
              : null
            : (existing[0].featured_image_drive_url ?? null),
        featured_image_url:

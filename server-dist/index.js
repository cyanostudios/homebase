...
function requirePlugin(pluginName) {
    return async (req, res, next) => {
        ...
        // Check plugin access
        const result = await pool.query('SELECT enabled FROM user_plugin_access WHERE user_id = $1 AND plugin_name = $2', [req.session.user.id, pluginName]);
...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        const pluginAccess = await pool.query('SELECT plugin_name FROM user_plugin_access WHERE user_id = $1 AND enabled = true', [user.id]);
        req.session.user = {
            id: user.id,
            email: user.email,
            role: user.role,
...
app.get('/api/contacts', requirePlugin('contacts'), async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM contacts WHERE user_id = $1 ORDER BY contact_number', [req.session.user.id]);
...
app.delete('/api/contacts/:id', requirePlugin('contacts'), async (req, res) => {
    try {
        const contactId = req.params.id;
        const result = await pool.query('DELETE FROM contacts WHERE id = $1 AND user_id = $2 RETURNING id', [contactId, req.session.user.id]);
        if (!result.rows.length) {
            return res.status(404).json({ error: 'Contact not found' });
        }
...
app.get('/api/notes', requirePlugin('notes'), async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM notes WHERE user_id = $1 ORDER BY created_at DESC', [req.session.user.id]);
...
app.delete('/api/notes/:id', requirePlugin('notes'), async (req, res) => {
    try {
        const noteId = req.params.id;
        const result = await pool.query('DELETE FROM notes WHERE id = $1 AND user_id = $2 RETURNING id', [noteId, req.session.user.id]);
        if (!result.rows.length) {
            return res.status(404).json({ error: 'Note not found' });
        }

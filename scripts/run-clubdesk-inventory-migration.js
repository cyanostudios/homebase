#!/usr/bin/env node
// scripts/run-clubdesk-inventory-migration.js
// Run server/migrations/171-clubdesk-inventory.sql on all tenant databases (local-first).
...
const MIGRATION_FILE = path.join(__dirname, '../server/migrations/171-clubdesk-inventory.sql');
...
async function main() {
  ...
  try {
    ...
    console.log(`Applying 171-clubdesk-inventory.sql to ${tenants.length} tenant(s)...`);

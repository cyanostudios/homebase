#!/usr/bin/env node
// scripts/run-instructions-migration.js
// Run instructions migrations: 114 + 116 + 117 (tenant) + 115 grant (main)
...
const TENANT_MIGRATIONS = [
  path.join(__dirname, '../server/migrations/114-instructions.sql'),
  path.join(__dirname, '../server/migrations/116-instructions-sort-order.sql'),
  path.join(__dirname, '../server/migrations/117-instruction-categories.sql'),
];
const MAIN_MIGRATION = path.join(
  __dirname,
  '../server/migrations/115-grant-instructions-plugin-access.sql',
);
...
async function runMainGrant(connectionString, label) {
  ...
  try {
    ...
    console.log(`✅ Grant instructions plugin access applied (${label})`);
    return { success: true };
  } catch (error) {
...
async function main() {
  ...
  try {
    ...
    // Main DB grant (local DATABASE_URL). Alternative: npm run set:tenant-plugins -- --enable=instructions

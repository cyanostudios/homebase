#!/usr/bin/env node
// scripts/run-cups-visible-migration.js
// Run server/migrations/062-cups-visible.sql on all tenant databases.
...
const MIGRATION_FILE = path.join(__dirname, '../server/migrations/062-cups-visible.sql');
...
async function runMigrationOnTenant(connectionString, tenantInfo) {
  ...
  try {
    ...
    console.log(`\n📦 Running cups visible migration on: ${tenantLabel}...`);

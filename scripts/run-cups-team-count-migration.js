#!/usr/bin/env node
// scripts/run-cups-team-count-migration.js
// Run server/migrations/061-cups-match-format-team-count.sql on all tenant databases.
...
const MIGRATION_FILE = path.join(
  __dirname,
  '../server/migrations/061-cups-match-format-team-count.sql',
);
...
async function runMigrationOnTenant(connectionString, tenantInfo) {
  ...
  try {
    ...
    console.log(`\n📦 Running cups team_count / match_format migration on: ${tenantLabel}...`);
    ...
    if (
      ...
      console.log(`   ⚠️  Column or object already exists (skip)`);

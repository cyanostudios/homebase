...
async function main() {
  ...
  try {
    ...
    for (const schema of tenantSchemas) {
      ...
      console.log(`\n--- ${schema} ---`);
      console.log(
        '  022 channel_product_map.channel_instance_id:',
        status['022_channel_product_map.channel_instance_id'],
      );
      console.log(
        '  022 orders.channel_instance_id:             ',
        status['022_orders.channel_instance_id'],
      );
      console.log('  030 ux_channel_product_map_..._instance:    ', status['030_unique_index']);
      console.log(
        '  031 products.channel_specific:              ',
        status['031_products.channel_specific'],
      );
    }

    console.log('\n\n📌 Tolkning:');
    console.log(
      '  - Om 022 visar NEJ: Kolumnerna saknas. Kör "node scripts/run-all-migrations.js"',
    );
    console.log(
      '  - 022 använder ADD COLUMN IF NOT EXISTS, så den går att köra även om delvis applicerad',
    );

#!/usr/bin/env node
// scripts/run-cups-ratings-club-migration.js
// Run server/migrations/074-cup-ratings-add-club.sql on all tenant databases.
...
const MIGRATION_FILE = path.join(__dirname, '../server/migrations/074-cup-ratings-add-club.sql');
...
async function main() {
  ...
  try {
    ...
    console.log(`Running migration on ${tenants.length} tenant(s)...`);
    let ok = 0;
    let skipped = 0;
    let failed = 0;
    ...
    for (const tenant of tenants) {
      ...
      if (result.skipped) {
        skipped += 1;
        console.log(`  [skip] ${tenant.email}`);
      } else if (result.success) {
        ok += 1;
        console.log(`  [ok]   ${tenant.email}`);
      } else {
        failed += 1;
        console.error(`  [fail] ${tenant.email}: ${result.error}`);
      }
    }

#!/usr/bin/env node
// Apply 166-tenants-default-texts.sql on main DB (local and/or prod).
...
const MIGRATION_FILE = path.join(
  __dirname,
  '..',
  'server/migrations/166-tenants-default-texts.sql',
);
...
async function apply(label, connectionString) {
  ...
  try {
    ...
    console.log(`✅ tenants.default_texts migration applied (${label})`);
  } finally {
    await pool.end();
  }
}

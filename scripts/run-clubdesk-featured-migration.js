#!/usr/bin/env node
// scripts/run-clubdesk-featured-migration.js
// Run server/migrations/127-clubdesk-featured.sql on all tenant databases.
...
const MIGRATION_FILE = path.join(__dirname, '../server/migrations/127-clubdesk-featured.sql');
...
async function main() {
  ...
  try {
    ...
    console.log(`Applying 127-clubdesk-featured.sql to ${tenants.length} tenant(s)...`);

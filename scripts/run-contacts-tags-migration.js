#!/usr/bin/env node
// scripts/run-contacts-tags-migration.js
// Run contacts tags migration (009-add-tags-to-contacts.sql) on all existing tenant databases
...
const MIGRATION_FILE = path.join(__dirname, '../server/migrations/009-add-tags-to-contacts.sql');
...
async function runMigrationOnTenant(connectionString, tenantInfo) {
  ...
  try {
    ...
    // Idempotent: IF NOT EXISTS should prevent most failures, but keep safe handling

#!/usr/bin/env node
// scripts/run-rename-slots-table-migration.js
// Run migration 035 (rename kiosk_slots table to slots) on all existing tenant databases
...
const MIGRATION_FILE = path.join(
  __dirname,
  '../server/migrations/035-rename-kiosk-slots-table-to-slots.sql',
);
...
async function runMigrationOnTenant(connectionString, tenantInfo) {
  ...
  try {
    ...
    if (
      ...
      console.log('   Table/index already renamed (migration already run)');

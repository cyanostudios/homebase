#!/usr/bin/env node
// scripts/run-invoices-order-delivery-migration.js
// Run 160-invoices-order-delivery.sql on all tenant databases.
...
const MIGRATION_FILE = path.join(__dirname, '../server/migrations/160-invoices-order-delivery.sql');
  ...
  try {
    ...
    if (isLocalProvider) {
      ...
      const result = await mainPool.query(`
        ...
        ORDER BY t.user_id
      `);
      // Local Neon-parity: connection string points at localhost — data often lives in public.
      tenants = result.rows.map((row) => {
        const conn = row.connection_string || '';
        const isLocalConn = /localhost|127\.0\.0\.1/i.test(conn);
        return {
          user_id: row.user_id,
          email: row.email,
          connection_string: isLocalConn ? targetUrl : conn,
          schema_name: null,
        };
      });
    }

#!/usr/bin/env node
// scripts/run-clubdesk-info-contacts-migration.js
// Run server/migrations/128-clubdesk-info-contacts.sql on all tenant databases.
...
const MIGRATION_FILE = path.join(__dirname, '../server/migrations/128-clubdesk-info-contacts.sql');
...
async function main() {
  ...
  try {
    ...
    console.log(`Applying 128-clubdesk-info-contacts.sql to ${tenants.length} tenant(s)...`);

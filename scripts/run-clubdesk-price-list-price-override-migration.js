#!/usr/bin/env node
// scripts/run-clubdesk-price-list-price-override-migration.js
// Run server/migrations/173-clubdesk-price-list-price-override.sql on all tenant DBs (local-first).
...
const MIGRATION_FILE = path.join(
  __dirname,
  '../server/migrations/173-clubdesk-price-list-price-override.sql',
);
...
async function main() {
  ...
  try {
    ...
    console.log(
      `Applying 173-clubdesk-price-list-price-override.sql to ${tenants.length} tenant(s)...`,
    );
    ...
    for (const tenant of tenants) {
      ...
      const label = tenant.email || tenant.user_id;
      if (result.success) {
        console.log(`  ${result.skipped ? 'Skipped' : 'Applied'}: ${label}`);
      } else {
        failed += 1;

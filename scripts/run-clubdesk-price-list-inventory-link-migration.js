#!/usr/bin/env node
// scripts/run-clubdesk-price-list-inventory-link-migration.js
// Run server/migrations/172-clubdesk-price-list-inventory-link.sql on all tenant DBs (local-first).
...
const MIGRATION_FILE = path.join(
  __dirname,
  '../server/migrations/172-clubdesk-price-list-inventory-link.sql',
);
...
async function main() {
  ...
  try {
    ...
    console.log(
      `Applying 172-clubdesk-price-list-inventory-link.sql to ${tenants.length} tenant(s)...`,
    );

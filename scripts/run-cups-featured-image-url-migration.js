#!/usr/bin/env node
// scripts/run-cups-featured-image-url-migration.js
// Run server/migrations/071-cups-featured-image-url.sql on all tenant databases.
...
const MIGRATION_FILE = path.join(__dirname, '../server/migrations/071-cups-featured-image-url.sql');
...
async function main() {
  ...
  try {
    ...
    for (const tenant of tenants) {
      ...
      const r = await runMigrationOnTenant(connectionString, {
        userId: tenant.user_id,
        email: tenant.email,
        schemaName: tenant.schema_name,
      });
      if (!r.success) {
        console.error('Migration failed', r.tenantInfo, r.error);
        process.exit(1);
      }
      console.log('OK', tenant.user_id, tenant.email || '', r.skipped ? '(skipped)' : '');
    }
  } finally {
    await mainPool.end();
  }

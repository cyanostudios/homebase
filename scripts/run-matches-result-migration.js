#!/usr/bin/env node
// scripts/run-matches-result-migration.js
// Run 089 migration on all tenant databases
...
const MIGRATION_FILE = path.join(
  __dirname,
  '../server/migrations/089-matches-add-result-status.sql',
);
...
async function runMigrationOnTenant(connectionString, tenantInfo) {
  ...
  try {
    ...
    console.log(`   Applied ${path.basename(MIGRATION_FILE)}`);

#!/usr/bin/env node
// scripts/run-contact-time-entries-migration.js
// Run contact_time_entries migration (027-contact-time-entries.sql) on all existing tenant databases
...
const MIGRATION_FILE = path.join(__dirname, '../server/migrations/027-contact-time-entries.sql');
...
async function runMigrationOnTenant(connectionString, tenantInfo) {
  ...
  try {
    ...
    if (
      ...
      console.log(`   ⚠️  Table/index already exists (migration already run)`);

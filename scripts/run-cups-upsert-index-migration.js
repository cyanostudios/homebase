#!/usr/bin/env node
// scripts/run-cups-upsert-index-migration.js
// Run server/migrations/060-cups-upsert-index.sql on all tenant databases (partial unique index for cups import upsert).
...
const MIGRATION_FILE = path.join(__dirname, '../server/migrations/060-cups-upsert-index.sql');
...
async function runMigrationOnTenant(connectionString, tenantInfo) {
  ...
  try {
    ...
    console.log(`\n📦 Running cups upsert index migration on tenant: ${tenantLabel}...`);
    ...
    if (
      ...
      console.log(`   ⚠️  Index already exists (skip)`);

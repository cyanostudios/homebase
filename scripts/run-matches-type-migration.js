#!/usr/bin/env node
// scripts/run-matches-type-migration.js
// Run matches match_type migration (041-matches-add-match-type.sql) on all tenant databases
...
const MIGRATION_FILE = path.join(__dirname, '../server/migrations/041-matches-add-match-type.sql');
...
async function runMigrationOnTenant(connectionString, tenantInfo) {
  ...
  try {
    ...
    if (
      ...
      console.log('   Table/index already exists (migration already run)');

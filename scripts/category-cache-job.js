...
const {
  fetchCategoriesFromApi: fetchCdonCategories,
} = require('../plugins/cdon-products/fetchCategories');
const {
  fetchCategoriesFromApi: fetchFyndiqCategories,
} = require('../plugins/fyndiq-products/fetchCategories');
const {
  fetchCategoriesFromApi: fetchWooCategories,
} = require('../plugins/woocommerce-products/fetchCategories');
...
async function tenantSchemaHasRequiredTables(client, schemaName) {
  ...
  return REQUIRED_TENANT_TABLES.filter((tableName) => !existing.has(tableName));
}
...
async function runJobForTenant(connectionString, tenantInfo) {
  ...
  try {
    ...
    if (missingTables.length > 0) {
      console.warn(
        `   [${tenantInfo.email}] Skipping category cache for ${schemaName || 'current schema'}; missing tables: ${missingTables.join(', ')}`,
      );
      return;
    }
    ...
    const cdonApiSecret = cdonSettings?.api_secret
      ? CredentialsCrypto.decrypt(cdonSettings.api_secret)
      : '';
    if (cdonApiKey && cdonApiSecret) {
      const cdonCacheKey = `cdon:categories:${categoryLanguage}`;
      try {
        const items = await fetchCdonCategories(
          market,
          categoryLanguage,
          cdonApiKey,
          cdonApiSecret,
    ...
    const fyndiqApiKey = fyndiqSettings?.api_key
      ? CredentialsCrypto.decrypt(fyndiqSettings.api_key)
      : '';
    const fyndiqApiSecret = fyndiqSettings?.api_secret
      ? CredentialsCrypto.decrypt(fyndiqSettings.api_secret)
      : '';
    if (fyndiqApiKey && fyndiqApiSecret) {
      const fyndiqCacheKey = `fyndiq:categories:${categoryLanguage}`;
      try {
        const items = await fetchFyndiqCategories(
          marketLower,
          categoryLanguage,
          fyndiqApiKey,
          fyndiqApiSecret,

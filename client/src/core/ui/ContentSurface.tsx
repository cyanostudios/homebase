...
/** Shared list + detail panel chrome (gray surface, rounded, full height). */
export const MAIN_CONTENT_SHELL_CLASS =
  'flex h-full min-h-0 w-full flex-col overflow-hidden rounded-xl border-0 bg-background';
...
export function ContentSurface({ children, className, flush = false }: ContentSurfaceProps) {
  return (
    <div className={cn('relative m-0 flex h-full min-h-0 min-w-0 w-full flex-col', className)}>
      <Card
        className={cn(
          'relative flex h-full min-h-0 w-full flex-col overflow-hidden rounded-none border-0 bg-card shadow-none',

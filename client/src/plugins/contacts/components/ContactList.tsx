  ...
  {searchTerm && sortedContacts.length !== contacts.length && ` of ${contacts.length}`})
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:items-center">
          ...
          <div className="flex gap-2">
            <Button onClick={handleOpenPanel} variant="primary" icon={Plus}>Add Contact</Button>
          </div>
        </div>
      </div>
              ...
              {sortedContacts.length === 0 ? (
                ...
                sortedContacts.map((contact, idx) => (
                  <tr 
                    ...
                    onClick={(e) => { e.preventDefault(); handleOpenForView(contact); }}
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        {contact.contactType === 'company' ? (<Building className="w-5 h-5 text-blue-500" />) : (<User className="w-5 h-5 text-green-500" />)}
                        <div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Badge className={contact.contactType === 'company' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="sm" icon={Eye}
                          onClick={(e) => { e.stopPropagation(); handleOpenForView(contact); }}>
                          View
                        </Button>
                        <Button variant="secondary" size="sm" icon={Edit}
                          onClick={(e) => { e.stopPropagation(); handleOpenForEdit(contact); }}>
                          Edit
                        </Button>
                      </div>
                    </td>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="space-y-1">
                        ...
                        {contact.website && <div className="text-xs text-gray-600">{contact.website}</div>}
                        {contact.phone && <div className="text-xs text-gray-600">{contact.phone}</div>}
                      </div>
                    </div>
                    <div>
                      <Button variant="ghost" size="sm" icon={Eye} onClick={() => handleOpenForView(contact)} className="h-8 px-3">
                        View
                      </Button>
                    </div>
                  </div>

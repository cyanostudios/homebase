...
export type InventoryListTableProps = {
  ...
  onRowClick: (item: ClubdeskInventoryItem) => void;
...
export function InventoryListTable({
          ...
          return (
            <div className="flex min-w-0 flex-col gap-0.5">
              <div className="flex min-w-0 items-center gap-1.5">
                <span title={t('nav.clubdesk-inventory')} className="inline-flex shrink-0">
                  <SectionCategoryIcon
                    icon={ShoppingBag}
  ...
  return (
    <SortableListTable
      ...
      rowAriaLabel={(item) =>
        t('clubdesk.inventory.openInventory', { name: item.articleName || item.id })
      }
      isRowActive={(item) =>

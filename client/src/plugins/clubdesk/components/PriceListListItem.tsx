...
export function PriceListListItem({
  priceList,
  selected,
  highlighted,
  onClick,
  ...
  const excerpt = priceList.description ? truncateText(priceList.description) : '';
  const updatedLabel = priceList.updatedAt
    ? new Date(priceList.updatedAt).toLocaleDateString()
    : null;
  const itemCount = priceList.itemCount ?? priceList.items?.length ?? 0;
  const isPublished = priceList.publicationStatus === 'published';
  ...
  const metaRow = (
    ...
    >
      <span className="truncate">{t('clubdesk.priceList.itemCount', { count: itemCount })}</span>
      <span className="truncate">{priceList.currency || 'SEK'}</span>
      {updatedLabel ? (
  ...
  return (
    <Card
      ...
      data-list-item={JSON.stringify(priceList)}
      data-plugin-name="clubdesk"
      role="button"
      tabIndex={0}
      aria-label={t('clubdesk.priceList.openPriceList', { title: priceList.title })}
    >
      <div className="flex flex-col gap-2 p-4">
        <div className="flex min-w-0 items-start justify-between gap-2">
          ...
          <div className="flex flex-shrink-0 items-center gap-1">
            {onStatusChange ? (
              ...
              >
                <Select
                  value={priceList.publicationStatus}
            ...
            {onFeaturedChange ? (
              ...
              >
                <Select
                  value={priceList.featured === true ? 'true' : 'false'}
            ...
            {canReorder ? (
              ...
              >
                <Button
                  ...
                  aria-label={t('clubdesk.priceList.moveUp')}
                  onClick={() => onMoveUp?.()}
                />
                <Button
                  type="button"
                  ...
                  aria-label={t('clubdesk.priceList.moveDown')}
                  onClick={() => onMoveDown?.()}
                />
              </div>
            ) : null}
        ...
        <h3 className={cn('line-clamp-2', DETAIL_LIST_ITEM_TITLE_CLASS)}>{priceList.title}</h3>

/** Public share URL for an estimate token (same path as Share dialog / ShareBlock). */
export function buildEstimateShareUrl(
  shareToken: string,
  origin: string = typeof window !== 'undefined' ? window.location.origin : '',
): string {
  return `${origin}/public/estimate/${shareToken}`;
}

/** Plain-text block appended to the email body via BulkEmailDialog.additionalText. */
export function formatEstimateShareEmailText(shareUrl: string, label: string): string {
  return `---\n${label}:\n${shareUrl}`;
}

/** HTML block appended to the email body via BulkEmailDialog.additionalHtml. */
export function formatEstimateShareEmailHtml(shareUrl: string, label: string): string {

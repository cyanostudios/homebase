...
/**
 * Open estimate document HTML in a new browser tab/window (shared-style full preview).
 * Call `window.open` synchronously from the click handler, then write HTML when ready.
 */
export function openEstimatePreviewWindow(): Window | null {
  ...
  try {
    win.document.open();
    win.document.write(
      '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Offert</title></head>' +
...
export function writeEstimatePreviewWindow(
  win: Window,
  html: string,
  title?: string,

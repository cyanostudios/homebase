  ...
  return (
    <div className="space-y-4">
      ...
      <Card padding="sm" className="shadow-none px-0">
        <Heading level={3} className="mb-3 text-sm font-semibold text-foreground">Invoice</Heading>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
          ...
          <div>
            ...
            <div className="text-foreground">{issueDate ? issueDate.toLocaleDateString() : '—'}</div>
          </div>
          <div>
      ...
      {actualItem.lineItems && actualItem.lineItems.length > 0 && (
        <Card padding="sm" className="shadow-none px-0">
          <Heading level={3} className="mb-3 text-sm font-semibold text-foreground">
                  ...
                  <tr key={index} className="hover:bg-accent transition-colors">
                    <td className="px-4 py-3">
                      ...
                      {lineItem.description && lineItem.name && (
                        <div className="text-xs text-muted-foreground">{lineItem.description}</div>
                      )}
                    </td>
      ...
      {/* Totals */}
      <Card padding="sm" className="shadow-none px-0">
        <Heading level={3} className="mb-3 text-sm font-semibold text-foreground">Totals</Heading>
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            <span className="text-muted-foreground">Subtotal</span>
            <span className="text-foreground font-medium">{Number(actualItem.subtotal ?? 0).toFixed(2)} {currency}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">Line Discounts</span>
            <span className="text-foreground font-medium">-{Number(actualItem.totalDiscount ?? 0).toFixed(2)} {currency}</span>
          </div>
          <div className="flex justify-between border-t border-border pt-2">
            <span className="text-muted-foreground">Subtotal after line discounts</span>
            <span className="text-foreground font-medium">{Number(actualItem.subtotalAfterDiscount ?? 0).toFixed(2)} {currency}</span>
          </div>
          {Number(actualItem.invoiceDiscount ?? 0) > 0 && (
            <>
              <div className="flex justify-between">
                ...
                <span className="text-foreground font-medium">-{Number(actualItem.invoiceDiscountAmount ?? 0).toFixed(2)} {currency}</span>
              </div>
              <div className="flex justify-between border-t border-border pt-2">
                <span className="text-muted-foreground">Subtotal after invoice discount</span>
                <span className="text-foreground font-medium">{Number(actualItem.subtotalAfterInvoiceDiscount ?? 0).toFixed(2)} {currency}</span>
              </div>
            </>
          )}
          <div className="flex justify-between">
            <span className="text-muted-foreground">Total VAT</span>
            <span className="text-foreground font-medium">{Number(actualItem.totalVat ?? 0).toFixed(2)} {currency}</span>
          </div>
          <div className="flex justify-between text-lg font-semibold border-t border-border pt-2">
            <span className="text-foreground">Total</span>
            <span className="text-foreground">{Number(actualItem.total ?? 0).toFixed(2)} {currency}</span>
          </div>
        </div>
      </Card>
      ...
      {(actualItem.paymentTerms || actualItem.notes) && (
        <>
          ...
          <Card padding="sm" className="shadow-none px-0">
            <Heading level={3} className="mb-3 text-sm font-semibold text-foreground">Payment & Notes</Heading>
              ...
              {actualItem.notes && (
                <div className="sm:col-span-2">
                  ...
                  <div className="bg-muted border border-border rounded-lg p-4">
                    <div className="text-sm text-foreground whitespace-pre-wrap">{actualItem.notes}</div>
                  </div>
                </div>
              )}
            </div>
      ...
      {/* Quick Actions */}
      <Card padding="sm" className="shadow-none px-0">
        <Heading level={3} className="mb-3 text-sm font-semibold text-foreground">Quick Actions</Heading>
      ...
      {/* Metadata */}
      <Card padding="sm" className="shadow-none px-0">
        <Heading level={3} className="mb-3 text-sm font-semibold text-foreground">Invoice Information</Heading>
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          <div>
            <div className="text-xs text-muted-foreground">System ID</div>
            <div className="text-sm font-mono text-foreground">{String(actualItem.id ?? '—')}</div>
          </div>
          <div>

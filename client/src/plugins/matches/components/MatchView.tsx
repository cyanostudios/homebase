...
/** Schedule and identity fields (name, teams, time, location). */
function MatchInformationCard({ match }: Pick<MatchMainInfoCardProps, 'match'>) {
...
/** Sport, format, competition, result, and status fields. */
function MatchPropertiesCard({ match, sportLabel }: MatchMainInfoCardProps) {
  const { t } = useTranslation();
...
export function MatchView({ match: matchProp, item, stacked: _stacked = false }: MatchViewProps) {
  ...
  const activeTab = parseMatchViewTab(searchParams.get('tab'));
  ...
  const contactCount = displayMentions?.length ?? 0;
  const relatedSlotCount = relatedSlots.length;
  ...
  const tabs = useMemo(
    () => [
      ...
      },
      {
        ...
        icon: Users,
        count: contactCount > 0 ? contactCount : null,
      },
      {
        id: 'linked' as const,
    ...
    relatedSlots.length === 0 ? (
      <Card padding="none" className={DETAIL_VIEW_CARD_CLASS}>
        <DetailSection
          ...
          className="p-6"
        >
          <p className={DETAIL_EMPTY_STATE_CLASS}>{t('matches.tabs.linkedEmpty')}</p>
        </DetailSection>
      </Card>
    ) : (
      <RelatedItemsCard

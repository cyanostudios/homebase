...
export const ProductView: React.FC<ProductViewProps> = ({ item, contact }) => {
  // Normalize to product with MVP fields + legacy fallbacks
  ...
  return (
    <div className="space-y-4">
      ...
      <Card padding="sm" className="shadow-none px-0">
        <Heading level={3} className="mb-3">Product Summary</Heading>
              ...
              {product.categories?.length ? (
                <ul className="flex flex-wrap gap-2">
                  {product.categories.map((c: string, i: number) => (
                    <li key={`${c}-${i}`} className="px-2 py-0.5 rounded-full border text-xs">{c}</li>
                  ))}
                </ul>
              ) : (
      ...
      {product.images?.length > 0 && (
        <Card padding="sm" className="shadow-none px-0">
          <Heading level={3} className="mb-3">Images</Heading>
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3">
            {product.images.map((url: string, i: number) => (
              <a key={`${url}-${i}`} href={url} target="_blank" rel="noopener noreferrer" className="block">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={url} alt={`Product image ${i + 1}`} className="w-full h-24 object-cover rounded-md border" />
              </a>
            ))}
          </div>
        </Card>
      ...
      <Card padding="sm" className="shadow-none px-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <div className="text-sm text-gray-900">{product.createdAt ? new Date(product.createdAt).toLocaleDateString() : '—'}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500">Last Updated</div>
            <div className="text-sm text-gray-900">{product.updatedAt ? new Date(product.updatedAt).toLocaleDateString() : '—'}</div>
          </div>
        </div>
      </Card>
    </div>

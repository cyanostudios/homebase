...
class CdonApi {
  ...
  async bulkCreateArticles(payload: { articles: Record<string, unknown>[] }): Promise<{ success?: unknown[]; failed?: unknown[] }> {
    return this.request('/articles/bulk', { method: 'POST', body: JSON.stringify(payload) });
  }

  async bulkUpdateArticles(payload: { actions: Array<{ sku: string; action: string; body?: Record<string, unknown> }> }): Promise<{ success?: unknown[]; failed?: unknown[] }> {

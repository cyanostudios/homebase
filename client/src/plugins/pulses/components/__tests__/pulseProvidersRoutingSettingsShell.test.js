...
describe('PulseProvidersRouting settings shell', () => {
  test('uses PluginSettingsPageShell with stacked global + per-plugin cards (no category tabs)', () => {
    ...
    expect(routingSrc).toMatch(/pulses\.routing\.globalTitle/);
    expect(routingSrc).toMatch(/pulses\.routing\.pluginsTitle/);
    expect(routingSrc).toMatch(/icon=\{Sparkles\}/);
    expect(routingSrc).toMatch(/icon=\{Route\}/);
    expect(routingSrc).toMatch(/iconPlugin="pulses"/);
  ...
  test('per-plugin list uses compact invoice-style rows', () => {
    expect(routingSrc).toMatch(/from '@\/components\/ui\/switch'/);
    expect(routingSrc).toMatch(/handleTogglePluginEnabled/);

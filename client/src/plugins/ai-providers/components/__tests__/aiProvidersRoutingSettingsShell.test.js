...
const routingSrc = fs.readFileSync(path.join(__dirname, '../AIProvidersRouting.tsx'), 'utf8');
const listSrc = fs.readFileSync(path.join(__dirname, '../AIProvidersList.tsx'), 'utf8');

describe('AIProvidersRouting settings shell', () => {
  test('uses PluginSettingsPageShell with stacked global + per-plugin cards (no category tabs)', () => {
    ...
    expect(routingSrc).toMatch(/aiProviders\.routing\.globalTitle/);
    expect(routingSrc).toMatch(/aiProviders\.routing\.pluginsTitle/);
    expect(routingSrc).toMatch(/icon=\{Sparkles\}/);
    expect(routingSrc).toMatch(/icon=\{Route\}/);
    expect(routingSrc).toMatch(/iconPlugin="ai-providers"/);

...
function getNoteExportFilename(note: Note, extension: string): string {
  return `${getNoteExportBaseFilename(note)}.${extension}`;
}

function noteToCsvRow(note: Note): Record<string, any> {
  return {
    title: note.title || '',
...
function noteToPdfRow(note: Note): Record<string, any> {
  return {
    title: note.title || '',

import { File, Mail, Pencil, Plus, Trash2 } from 'lucide-react';
...
export const InspectionView: React.FC<InspectionViewProps> = (props) => {
  ...
  const { loadProjects, openInspectionForEdit } = useInspections();
  ...
  const handleSetFiles = async (fileIds: string[]) => {
    try {
      await inspectionApi.setFiles(project.id, fileIds);
      await refresh();
  ...
  return (
    <div className="p-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Vänster: Beskrivning, admin-kommentarer, Redigera och Skicka-knapp */}
        <div className="space-y-4 min-w-0">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => openInspectionForEdit(project)}>
              <Pencil className="h-4 w-4 mr-2" />
              Redigera
            </Button>
          </div>
          <Button className="w-full" variant="default" onClick={() => setShowSendModal(true)}>
            <Mail className="h-4 w-4 mr-2" />
            Skicka till hantverkare
          </Button>
        </div>

        {/* Höger: Bifogade filer + filväljare */}

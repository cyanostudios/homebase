...
const statsSrc = fs.readFileSync(
  path.join(__dirname, '../MailProvidersStatisticsView.tsx'),
  'utf8',
);
const formSrc = fs.readFileSync(path.join(__dirname, '../MailSettingsForm.tsx'), 'utf8');
const registrySrc = fs.readFileSync(
  path.join(__dirname, '../../../../core/pluginRegistry.ts'),
  'utf8',
);

describe('MailProvidersList mail-layout list|content split', () => {
  test('list uses desktop 20/80 grid with statistics empty state', () => {
    ...
    expect(listSrc).toMatch(/MailProvidersStatisticsView/);
  ...
  test('registry owns scroll and content view key', () => {
    expect(registrySrc).toMatch(/name: 'mail'/);
    expect(registrySrc).toMatch(/contentOwnsScroll: true/);
    expect(registrySrc).toMatch(/contentViewKey: 'mailContentView'/);
  });

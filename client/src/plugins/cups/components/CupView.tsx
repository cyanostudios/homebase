import { Copy, ExternalLink, Info, SlidersHorizontal, Trash2, Zap } from 'lucide-react';
...
function renderRawWithLinks(raw: string) {
  ...
  return blocks.map((paragraph) => {
    const chunks = paragraph.split(urlRe);
    const paragraphKey = paragraph.slice(0, 80);
    return (
      <div
        key={`raw-${paragraphKey}`}
        className={cn(
          'pb-3',
          paragraph !== blocks[blocks.length - 1] && 'mb-3 border-b border-border/40',
        )}
      >
        <p className="whitespace-pre-wrap text-sm leading-relaxed text-foreground/90">
          {chunks.map((chunk) => {
            ...
            if (isUrl) {
              return (
                <a
                  key={`raw-url-${paragraphKey}-${chunk}`}
                  href={chunk}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="underline decoration-dotted underline-offset-2 text-plugin hover:opacity-80"
                >
                  {chunk}
                </a>
              );
            ...
            return (
              <React.Fragment key={`raw-text-${paragraphKey}-${chunk}`}>{chunk}</React.Fragment>
            );
          })}
        </p>
      </div>
...
function CupMetadataCard({ cup }: CupMetadataCardProps) {
  ...
  const sourceLabel = (() => {
    ...
    if (cup.source_url.startsWith('file://')) {
      const filename = cup.source_url
        .replace(/^file:\/\//, '')
        .split('/')
        .pop();
      return filename || cup.source_url;
    }
    try {
      const url = new URL(cup.source_url);
      return url.hostname.replace(/^www\./, '') || cup.source_url;
    } catch {
      return cup.source_url;
    }
  })();
  return (
    <Card padding="none" className={CUP_DETAIL_CARD_CLASS}>
      <DetailSection title={t('cups.information')} icon={Info} iconPlugin="cups" className="p-4">
        <div className="space-y-4 text-xs">
          ...
          {sourceLabel && (
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">{t('cups.source')}</span>
              <span className="max-w-[14rem] truncate text-right font-medium">{sourceLabel}</span>
            </div>
          )}
        </div>
      </DetailSection>
...
interface CupPropertiesCardProps {
  cup: Cup;
  onUpdate: (values: { visible: boolean; sanctioned: boolean }) => Promise<void>;
}

function CupPropertiesCard({ cup, onUpdate }: CupPropertiesCardProps) {
  const { t } = useTranslation();
  const [draftVisible, setDraftVisible] = useState(cup.visible !== false);
  const [draftSanctioned, setDraftSanctioned] = useState(cup.sanctioned === true);
  const [isSaving, setIsSaving] = useState(false);

  React.useEffect(() => {
    setDraftVisible(cup.visible !== false);
    setDraftSanctioned(cup.sanctioned === true);
  }, [cup.id, cup.visible, cup.sanctioned]);

  const isDirty = draftVisible !== (cup.visible !== false) || draftSanctioned !== (cup.sanctioned === true);
  ...
  const handleUpdate = async () => {
    ...
    setIsSaving(true);
    try {
      await onUpdate({ visible: draftVisible, sanctioned: draftSanctioned });
    } finally {
      setIsSaving(false);
    }
  };
      ...
      >
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-4">
            <span className="text-sm font-medium text-foreground">{t('common.visible')}</span>
            <Switch
          ...
          <div className="flex items-center justify-between gap-4">
            <span className="text-sm font-medium text-foreground">{t('cups.sanctioned')}</span>
            <Switch
              checked={draftSanctioned}
              onCheckedChange={(checked) => setDraftSanctioned(Boolean(checked))}
              className="h-4 w-7 data-[state=checked]:bg-primary [&>span]:h-3 [&>span]:w-3 [&[data-state=checked]>span]:translate-x-3"
            />
          </div>
          <div className="pt-1">
            ...
            >
              {isSaving ? t('common.saving') : t('common.update', { defaultValue: 'Update' })}
            </Button>
          </div>
        </div>
      </DetailSection>
    ...
    <Card padding="none" className={cn(CUP_DETAIL_CARD_CLASS, 'plugin-cups')}>
      <div className="space-y-5 p-6">
        <div>
          ...
          <div className="text-2xl font-semibold text-foreground">{cleanCupName(cup.name)}</div>
        </div>
        ...
        {raw && (
          <div className="border-t border-border/50 pt-5">
            ...
            <div className="max-h-[min(78vh,40rem)] min-h-[12rem] overflow-y-auto break-words rounded-lg border border-border/50 bg-muted/25 px-3 py-3 shadow-inner">
              <div className="space-y-0">{renderRawWithLinks(raw)}</div>
            </div>
          </div>
        )}
      </div>
...
export function CupView({ cup: cupProp, item }: CupViewProps) {
  ...
  const { currentCup, panelMode, openCupForEdit, closeCupPanel, deleteCup, saveCup } = useCups();
        ...
        <DetailLayout
          ...
          sidebar={
            <div className="space-y-4">
              ...
              <CupPropertiesCard
                cup={activeCup}
                onUpdate={async ({ visible, sanctioned }) => {
                  await saveCup({ visible, sanctioned });
                }}
              />

...
If the admin pool is empty, Cupappen uses only the JPEGs here (`fb01.jpg` … `fb25.jpg`). These are the built-in defaults — not admin uploads. Keep names in sync with `DEFAULT_FALLBACK_IMAGES` in `app.js` and the default list in `cup.php`.

...
import type { YourItemColumnCount } from '../utils/yourItemColumnCount';

export function YourItemListItem({
  item,
  selected,
  onClick,
  checkbox,
  ...
  columnCount?: YourItemColumnCount;
}) {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2 p-4">
        ...
        <h3 className={cn('line-clamp-2', DETAIL_LIST_ITEM_TITLE_CLASS)}>{item.title || '—'}</h3>
        {!metaOnTop ? metaRow : null}
      </div>
    </Card>
  );

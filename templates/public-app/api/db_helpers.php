...
/**
 * List SQL skeleton — assumes table `items` with visible + optional deleted_at.
...
function publicAppSitemapSql(PDO $pdo): string
{
    ...
    return <<<SQL
SELECT i.id, i.name, i.slug, i.updated_at
FROM items i
WHERE COALESCE(i.visible, TRUE) = TRUE
{$deletedFilter}
ORDER BY i.name ASC NULLS LAST, i.id ASC
...
function publicAppItemBySlugSql(PDO $pdo, string $slugOrId): array
{
    ...
    if (ctype_digit($slugOrId)) {
        return [
            'sql' => <<<SQL
SELECT i.id, i.name, i.slug, i.description, i.featured_image_url, i.updated_at
FROM items i
WHERE i.id = ?

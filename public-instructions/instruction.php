...
function parseItemPath(): ?string
{
    ...
    if (preg_match('#^/instruction/([a-z0-9-]+)/?$#i', $path, $matches)) {
...
if ($notFound || !$item) {
    ...
    $canonical = $baseUrl . '/instruction/' . ($itemSlug !== '' ? $itemSlug : $slug);
...
<html lang="sv">
  <head>
    ...
    <meta property="og:site_name" content="Instruktioner" />
<?php if ($ogImage !== ''): ?>

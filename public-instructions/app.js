/**
 * Public instructions listing client — AppShell design system.
 * List + detail use same-origin PHP (/api/items.php, /instruction/:slug).
 * Listing URLs: `/`, `/alla/`, `/info/`, `/kategori/:slug/`.
 * Optional override: window.PUBLIC_APP_API_URL
 */
...
const Urls = window.InstructionsListingUrls;

if (!Urls) {
  throw new Error('InstructionsListingUrls saknas — ladda /lib/listingUrls.js före app.js');
}
...
function itemHref(item) {
  ...
  if (slug) return `/instruction/${encodeURIComponent(slug)}`;
  return `/instruction/${encodeURIComponent(String(item.id || ''))}`;
}
...
function updateHeroCount(total) {
  ...
  heroCountLineEl.textContent = n === 1 ? '1 guide' : `${formatted} guider`;
}
...
/**
 * Hero band: home copy on home; quick-nav on listing tabs. Hidden on info.
 */
...
function renderInfoListing() {
  ...
  container.innerHTML = `
    <div class="info-panel" id="info">
      ...
      <h2 class="info-panel__title">Instruktioner</h2>
      <p class="info-panel__lead">
        Här hittar du steg-för-steg-guider för personalen. Välj en kategori på startsidan eller bläddra bland alla guider.
      </p>
      <p class="info-panel__contact">
        © <span id="footer-year">${escapeHtml(year)}</span> Instruktioner
      </p>
    </div>`;
}
...
function applyFilter() {
  ...
  setStatus(`${filtered.length} guider`);
  renderRows(filtered);
}

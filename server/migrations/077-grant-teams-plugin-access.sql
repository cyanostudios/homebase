-- 077-grant-teams-plugin-access.sql
-- MAIN_DB_ONLY
-- Grant plugin "teams" to all existing tenants and each tenant owner's user_plugin_access row.
...
BEGIN
  IF EXISTS (
    ...
    SELECT t.id, 'teams', true, COALESCE(t.owner_user_id, t.user_id)
    FROM tenants t
  ...
  IF EXISTS (
    ...
    SELECT DISTINCT x.uid, 'teams', true, x.uid
    FROM (

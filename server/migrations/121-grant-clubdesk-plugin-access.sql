-- 121-grant-clubdesk-plugin-access.sql
-- MAIN_DB_ONLY
-- Grant plugin "clubdesk" to all existing tenants and each tenant owner's user_plugin_access row.
-- Alternative: npm run set:tenant-plugins -- --enable=clubdesk
...
BEGIN
  IF EXISTS (
    ...
    SELECT t.id, 'clubdesk', true, COALESCE(t.owner_user_id, t.user_id)
    FROM tenants t
  ...
  IF EXISTS (
    ...
    SELECT DISTINCT x.uid, 'clubdesk', true, x.uid
    FROM (

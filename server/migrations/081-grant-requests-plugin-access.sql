-- 081-grant-requests-plugin-access.sql
-- MAIN_DB_ONLY
-- Grant plugin "requests" to all existing tenants and each tenant owner's user_plugin_access row.
...
BEGIN
  IF EXISTS (
    ...
    SELECT t.id, 'requests', true, COALESCE(t.owner_user_id, t.user_id)
    FROM tenants t
  ...
  IF EXISTS (
    ...
    SELECT DISTINCT x.uid, 'requests', true, x.uid
    FROM (

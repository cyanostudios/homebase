-- 091-grant-guides-plugin-access.sql
-- MAIN_DB_ONLY
-- Grant plugin "guides" to all existing tenants and each tenant owner's user_plugin_access row.
...
BEGIN
  IF EXISTS (
    ...
    SELECT t.id, 'guides', true, COALESCE(t.owner_user_id, t.user_id)
    FROM tenants t
  ...
  IF EXISTS (
    ...
    SELECT DISTINCT x.uid, 'guides', true, x.uid
    FROM (

-- 104-grant-places-plugin-access.sql
-- MAIN_DB_ONLY
-- Grant plugin "places" to all existing tenants and each tenant owner's user_plugin_access row.
-- Mirrors 091-grant-guides-plugin-access.sql.
...
BEGIN
  IF EXISTS (
    ...
    SELECT t.id, 'places', true, COALESCE(t.owner_user_id, t.user_id)
    FROM tenants t
  ...
  IF EXISTS (
    ...
    SELECT DISTINCT x.uid, 'places', true, x.uid
    FROM (

-- 168-grant-sportadmin-plugin-access.sql
-- MAIN_DB_ONLY
-- Grant plugin "sportadmin" to all existing tenants and each tenant owner's user_plugin_access row.
-- Idempotent: ON CONFLICT DO NOTHING. Local-first; prod enable also via set:tenant-plugins on release.
...
BEGIN
  IF EXISTS (
    ...
    SELECT t.id, 'sportadmin', true, COALESCE(t.owner_user_id, t.user_id)
    FROM tenants t
  ...
  IF EXISTS (
    ...
    SELECT DISTINCT x.uid, 'sportadmin', true, x.uid
    FROM (

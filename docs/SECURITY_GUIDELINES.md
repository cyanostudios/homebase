...
// ✅ REQUIRED: Verify that the record exists inside the active tenant
...
// Results in: SELECT _ FROM contacts WHERE email = '' OR '1'='1'
Prevention:
// ✅ Parameterized query
await database.query('SELECT _ FROM contacts WHERE email = ?', [email]);

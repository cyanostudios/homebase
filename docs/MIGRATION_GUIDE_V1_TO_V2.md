Overview
This guide helps you migrate existing Homebase plugins from V1 (Direct Infrastructure) to V2 (Service Abstraction + Security).
Migration Benefits:
...
Step 1.1: Add Core Service Imports
Before (V1):
...
module.exports = {
  ...
  security: {
    rateLimits: {
      ...
      email: { windowMs: 60 * 60 * 1000, max: 50 }
    },
    auditLog: true,
    csrfProtection: true
  }
...
async function migratePhotos() {
  ...
  const localDir = path.join(__dirname, '../uploads/contacts');

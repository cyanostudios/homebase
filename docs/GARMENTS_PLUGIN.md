...
The persons matrix is a **spreadsheet table**. Default visible identity column: **Name** (required). Extra identity fields (team / jersey name / initials / number) may be enabled later in code.

| Name _(required)_ | _(optional identity, code)_ | Paid _(default on new lists)_ | _(custom person checkboxes)_ | _(per assigned article)_ Audience · Size · Ordered · Delivered · Handed out |
...
- Pick a garment list, then edit **Person columns** (Paid + `custom_<uuid>`: hide, delete, reorder, add; **Add Paid** restores Paid if removed). Identity columns are **not** user-configurable (name-only default).

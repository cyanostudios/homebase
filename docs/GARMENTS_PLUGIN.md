...
- Updates via `PATCH …/ct-sizes` with optional `ctSizes` and/or `ctAudiences` (partial merge; only assigned item ids accepted server-side). Client sends **only changed keys** via `ctFieldPatch` (empty string clears). Soft-preview hydrates through `refreshGarmentList` so matrix edits sync into the detail pane.

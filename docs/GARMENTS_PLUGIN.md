...
| View          | URL                   | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Lists**     | `/garments`           | Clothing checklists: persons + spreadsheet (Paid by default on new lists; optional **custom** person-level checkbox columns; Ordered/Delivered/Handed out per **assigned** inventory article). List index: table-only mail layout. **Settings** (gear): per-list checkbox columns (see **Person rows**). Optional **list** team link (form). Person matrix: identity defaults to **name**, **jersey name**, **initials**, **jersey number** (team column coded but hidden by default); plus Paid/custom checkboxes. Inventory groups filtered to assigned articles only (legacy Shorts/Shirt/Socks and Blankett Fogis hidden).                                                                                                                                                                                                                                                              |
| **Inventory** | `/garments/inventory` | Stock **articles** with product fields (name required; optional brand, description, material, purchase/recommended/sale price, **tags**). Each article has **variants** (optional **line** + color and/or size + SKU + quantity; API field still `audience`) via a form repeater. List: table-only mail-layout (no sticky list QC). Full view first card is `InventoryQuickContextPanel` (variant list with inline qty, tags, assigned-list badges). Full view: **50/50** layout (`lg:grid-cols-2`) — details + **Show in lists** card beside variants; platform prev/next when more than one article. Assign articles to lists per item (see **Inventory ↔ lists**). List: tag filter chips; bulk **Tags** and **List visibility** via selection. **Settings** (gear): tag **catalog** + CSV/Excel/paste import (see **Import inventory**). Inventory list defaults to article name only. |
...
### Desktop inventory companion (right rail)

When the garments plugin is tenant-enabled, desktop shell (`lg+`, `AppRightSidebar`) can open **inventory only** as a global companion flyout (~640px), same platform pattern as Schedule.

| Topic            | Behaviour (verified in code)                                                                                                                                                                                                                                                                                                                                                                          |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Discovery**    | Registry `canOpenAsCompanionFor: ['teams']` (host list reserved/ignored — platform-wide when garments enabled).                                                                                                                                                                                                                                                                                       |
| **Rail**         | **`Package`** icon; label/title via **`nav.garments-inventory`** (`companionRailTitleNavPage`).                                                                                                                                                                                                                                                                                                       |
| **Hide / close** | On primary **`garments-inventory`** (`/garments/inventory`): rail toggle hidden; open flyout closes (`companionHideOnPrimaryPages` + `shouldCloseCompanionForPrimary` in `AppContent`). Still available on Lists (`/garments`) and other primary plugins.                                                                                                                                             |
| **Embed**        | `GarmentList` with `isCompanion` — forces **inventory** mode (not lists). **View-only:** no Add, select/bulk, settings, page title/gear, mobile search hook, or mail-toolbar portal. Row soft-select **replaces** the list with **`InventoryQuickContextPanel` `readOnly`**, **Open full item** (closes companion → `/garments/inventory` + `openInventoryForView`), and **Close** (returns to list). |
| **Columns**      | Same as full inventory: **`articleName` only** (brand · qty · price in identity meta).                                                                                                                                                                                                                                                                                                                |
| **Purpose**      | Keep inventory visible while working in another primary plugin; edits remain on the full inventory page.                                                                                                                                                                                                                                                                                              |
| **Backend**      | None — reuses existing garments APIs and auth. Security **Godkänt** 2026-09-23 (UI-only companion slice).                                                                                                                                                                                                                                                                                             |
| **Phone/pad**    | No right rail; companion not offered.                                                                                                                                                                                                                                                                                                                                                                 |

ADR: [`docs/ai/adr/GARMENTS_INVENTORY_COMPANION.md`](./ai/adr/GARMENTS_INVENTORY_COMPANION.md).
...
| `PATCH`  | `/api/garments/lists/:id/fit-summary-procurement`    | `{ "fitSummaryProcurement": { "<itemId>": { "<audience>\\u001f<size>": { "ordered": true, "qtyOrdered": 5 } } } }` — deep partial merge for Size summary supplier/batch progress. Breakdown key uses U+001F (not null byte; PostgreSQL JSONB rejects `\\u0000`). `qtyOrdered: null` clears qty. Item ids must be assigned to the list. Independent of person `inv_*_ordered`. |
...
The persons matrix is a **spreadsheet table**. Default visible identity columns: **Name** (required), **jersey name**, **initials**, **jersey number**. **Team** exists in code but is hidden by default.

| Name _(required)_ | Jersey name · Initials · Number _(default on)_ | Team _(code, hidden)_ | Paid _(default on new lists)_ | _(custom person checkboxes)_ | _(per assigned article)_ Line · Size · Ordered · Delivered · Handed out |

_(UI working language is English; Swedish in `sv.json`. Team is omitted when the teams plugin is disabled. Paid and custom checkboxes live in list `checkbox_columns`; inventory status columns come from assigned articles.)_
...
- Pick a garment list, then edit **Person columns** (Paid + `custom_<uuid>`: hide, delete, reorder, add; **Add Paid** restores Paid if removed). Identity columns are **not** user-configurable (code defaults: name + jersey name/initials/number; team hidden).
- **Paid** is included by default on **new** lists (`createDefaultCheckboxColumns`) and may be deleted from an existing list.
- Checkbox changes: `PUT /lists/:id` with the **full** `checkboxColumns` array (preserve `inv_*` / legacy / Fogis). Cap: **50** checkbox columns total (API).
...
**Filled field chrome (admin edit):** PersonMatrix cells, inventory quick-context quantity, and the contact-import tag select use shared `FORM_*` / `FORM_COMPACT_*` tokens from `formFieldStyles.ts` (`h-7`, borderless, muted). Jersey-duplicate emphasis uses amber **ring** (not border). See `PLUGIN_VIEW_IMPLEMENTATION_GUIDE.md` §3.

Below the spreadsheet (admin and public share), a **size summary** (`buildGarmentListFitSummary` / `GarmentListFitSummary`) lists each assigned inventory article in a **stacked** article group with line/size breakdowns and needed counts. Status checkboxes (Ordered/Delivered/Handed out) on the person matrix are **not** included in the counts.
...
- **Line** (stored as `audience` / `ct_audiences`): if the article’s variants define one or more distinct **line** values → dropdown per person (keyed by inventory item id); otherwise free-text input. Choosing **—** / clearing free text persists as empty (`""` in the PATCH body).
- **Size:** if the article’s variants define one or more **size** values → dropdown per person (stored in `garment_list_persons.ct_sizes`, keyed by inventory item id); otherwise free-text input. Same clear behaviour as line. Changing line clears size when the current size is not valid for the new line (also via `""`).
- Updates via `PATCH …/ct-sizes` with optional `ctSizes` and/or `ctAudiences` (partial merge; only assigned item ids accepted server-side). Client sends **only changed keys** via `ctFieldPatch` (empty string clears) and applies optimistic `patchPersonLocal` so the matrix updates immediately. Soft-preview hydrates through `refreshGarmentList` so matrix edits sync into the detail pane.
...
**Template columns (English labels; SV/EN aliases for auto-mapping):** Article _(required)_, Brand, Description, Material, Purchase Price, Recommended Price, Sale Price, Currency, Comment, Article no., Line _(aliases: Audience, Målgrupp, Linje)_, Color, Size, Quantity.
...
**Person team (Gate 5, 2026-08-31):** No unacceptable risks. Optional person `teamId` on create/update: numeric-or-null validation, CSRF, plugin gate, list `user_id` ownership. FK to tenant `teams(id)`; invalid id may surface as DB error (same class as list-level `team_id`). Public share may include numeric `teamId` in person JSON (UI does not show Team on public `PublicPersonMatrix`). Non-blocking follow-up: map FK violations to 400.

...
| **Right sidebar**    | Desktop only (`AppRightSidebar`). Fixed ~48px rail with round tools: User / Pomodoro / Timer open slide-outs; Settings goes to `/settings`. Phone/pad: rail hidden; user menu in TopBar. Plugin detail sidebars stay inline.                                                                                                                                                                                                                                                                                                                                                                                              |
...
- Rail tools (top → bottom): User, Theme, Settings, then a gap, then Pomodoro / Timer widgets. User / Pomodoro / Timer toggle flyouts; Settings navigates to `/settings` (via `navigateToSettings`); Theme toggles light/dark.
- Phone/pad: rail not shown; user menu stays in TopBar (`lg:hidden`). Plugin detail `sidebar` stays inline in `DetailLayout` (no portal into the rail).
- **Limits:** tools/flyouts are desktop-only. Pomodoro / timer / account prefs are not available in the TopBar.

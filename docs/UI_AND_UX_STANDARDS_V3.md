...
| **List toolbar** | Shared `ListToolbar` (`@/core/ui/ListToolbar`): one row. Idle (`selectedCount === 0` and not `selectionMode`): **Select all** + optional **Quick task/note/request** (`leadingActions`, left); right group = optional **`beforeSearch`** → **search** (`ListSearchInput` with clear **X** when non-empty) → sort + column **1/2/3/table** (`ListColumnLayoutToggle`). Quick-add open: form takes the whole row (`quickAddOpen` / `quickAddExpanded`; full-width input at normal control height `h-9`). Selection (`selectedCount > 0` **or** `selectionMode`): bulk actions replace that row (search/sort/columns hidden). Plugins with toolbar quick-add: Tasks, Notes, Requests (`*QuickAdd` + `layout="toolbar"`). **Phone:** entire `ListToolbar` card hidden (search stays mounted for bottom-bar registration; no sort/select-all). **Pad/desktop:** full toolbar with inline search. **Contacts exception (2026-08-27):** table-only list — **Select** / **Clear** round buttons in the page title row (`alwaysExpanded`); bulk via `BulkActionRoundBar` under the header (not `ListToolbar`). Checkboxes hidden until Select.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
**Contacts (table-only exception):** select/bulk lives in the list **page header** (`RoundExpandableSearch`, Select/Clear round buttons, then `BulkActionRoundBar` when `selectionMode`). Do not mount `ListToolbar` for Contacts bulk.

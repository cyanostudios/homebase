...
| **List toolbar** | Shared `ListToolbar` (`@/core/ui/ListToolbar`): one row. Idle (`selectedCount === 0`): **Select all** + optional **Quick task/note** (`leadingActions`, left); search beside sort + column **1/2/3** (right). Quick-add open: form takes the whole row (`quickAddOpen` / `quickAddExpanded`; full-width input at normal control height `h-9`). Selection (`selectedCount > 0`): bulk actions replace that row (search/sort/columns hidden).                                                                                                                                                                                                                                                                                                          |
| **List footer**  | Shared `ListFooterBar`: transparent surface (no white card); “Showing X of Y” left-aligned. Optional `leading` reserved; Tasks/Notes quick-add lives in the toolbar, not the footer.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |

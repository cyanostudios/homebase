...
- **Category picker (≥2 categories):** `SettingsCategoryCard` — same surface as Core Settings / `ListFilterStatCard`. Label row matches DetailSection `subtleTitle` / Contact Properties (`h-3.5` icon left + `text-xs font-bold uppercase tracking-[0.1em]`); short description under. No colored dot. Active: `bg-primary/10` + `text-primary` + `ring-1 ring-border/70`. Canonical icons from `SETTINGS_CATEGORY_ICONS` (`client/src/core/ui/settingsCategoryIcons.ts`): `view`→LayoutGrid, `import`→Upload, `tags`/`categories`→Tag, `api`→Settings2, `production`→Timer, `sources`→BookOpen. Desktop category grid: `grid-cols-1` + `md:grid-cols-N` by count (`PluginSettingsPageShell`).

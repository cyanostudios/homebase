...
- **Contacts full-view focus (trial):** `ContactView` opens with the same dim overlay on by default; contact cards + DetailPanel header/actions stay above (`DetailFocusOverlayContext`). Toggle / Esc / click outside exits. Hint: `contacts.focusModeHint`.

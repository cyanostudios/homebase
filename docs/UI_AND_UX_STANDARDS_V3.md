...
- **Shell content flush:** `ContentSurface` sits flush against the sidebar and top bar (`m-0`, `rounded-none`, `bg-card`). List and detail share `MAIN_CONTENT_SHELL_CLASS` (`rounded-xl bg-background`) so both use the same gray main surface and corner inset. `TopBar` is fixed with a matching flow spacer (`h-14`, or `h-[6.5rem]` on phone when the pomodoro/time-tracking tool row is shown). `MainLayout` `main` uses `lg:pl-[252px]` (permanent sidebar from desktop), right gutter `pr-4` from pad up, and phone-only `pb-16` / search `pb-28` so list content clears the phone bottom bar. Pad (768–1023) uses overlay nav + list|detail split — ADR [`ai/adr/VIEWPORT_TIER_PAD_SPLIT.md`](ai/adr/VIEWPORT_TIER_PAD_SPLIT.md). Internal list/detail padding (`p-4 md:p-6` / `flush`) is unchanged.
...
**Card-column list (rolled out 2026-07-24; table mode rolled out 2026-08-09; provider catalogs 2026-08-10)** — reference for **Tasks, Contacts, Notes, Guides, Requests, Slots, Estimates, Matches, Files, Ingest, Cups, Teams, Instructions, Clubdesk** (incl. price lists), **Mail providers, Pulses providers, AI Providers**:

| Element          | Standard                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- | --- | --- | --------------------- |
| **Layout**       | CSS grid of `*ListItem` cards using `DETAIL_VIEW_CARD_CLASS` (Teams may keep domain `TeamCard` visuals). **Table mode:** toolbar `1 \| 2 \| 3 \| table` via shared `ListColumnLayoutToggle`; `listViewMode: 'cards' \| 'table'`; table uses shared `SortableListTable` (`rowBorders={false}`, sortable headers). Persist `listViewMode` in plugin settings and/or session (`{plugin}:listViewMode`) — do **not** reuse legacy `viewMode`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Columns**      | Toolbar buttons **1 / 2 / 3** set `columnCount` (full / half / third from `sm`; one column below `sm`) and force cards. Persist `columnCount` (settings and/or session); migrate legacy `viewMode` grid→3, list→1. Table mode keeps last `columnCount` for return to cards. **Viewport tiers** (ADR `ai/adr/VIEWPORT_TIER_PAD_SPLIT.md`): **phone** (&lt;768) — cards + **1** grid column + card content as **column-2**; **pad** (768–1023) — cards + columns clamped to \*\*1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 2**; **desktop** (≥1024) — full preference including table. Overrides via `effectiveListViewMode.ts`; persisted preferences are not mutated. `ListColumnLayoutToggle`: hidden on phone; **1 | 2** on pad; **1 | 2   | 3   | table\*\* on desktop. |
...
| **List page header** | Title + Settings/Add is **`hidden md:flex`** (phone-hidden; visible on pad/desktop). Same chrome as detail: `items-start justify-between gap-4`, page shell `px-4 pt-2 pb-4 md:px-6 md:py-4`, title `text-xl` + subtitle `text-sm` stacked (`space-y-1`). Exceptions: Schedule / Core Settings as before.                                                                                                                                                                                                                                                                                                                 |
...
- **Category picker (≥2 categories):** `SettingsCategoryCard` stack (`grid-cols-1`) — same surface language as Core Settings / `ListFilterStatCard` (dot + uppercase label + icon + short description). Active: `bg-primary/10` + `text-primary` + `ring-1 ring-border/70` (same as filter-stat active). Use canonical icons from `SETTINGS_CATEGORY_ICONS` (`client/src/core/ui/settingsCategoryIcons.ts`): `view`→LayoutGrid, `import`→Upload, `tags`/`categories`→Tag, `api`→Settings2, `production`→Timer, `sources`→BookOpen. Category card glyph size: `h-5 w-5`.
...
- **Account profile** (`category: 'profile'`): shared account identity only — Account name, Contact (**Website**, **Email**, **Phone**), Logo, Address, Billing (incl. **Swish number**). No personal member fields.
...
- **Headings:** `text-xl font-semibold tracking-tight` (list + detail page headers; shared `PLUGIN_PAGE_TITLE_CLASS`)

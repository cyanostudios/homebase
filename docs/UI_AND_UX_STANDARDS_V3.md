...
**Mail-layout list|detail** (`contentOwnsScroll`, desktop ~20%|`1fr`, collapsible toolbar, **Filters** show/hide toggle next to Sort (`ListFilterChipsToggle`; on=white / off=`bg-primary/10`; persisted per plugin `localStorage` `'1'`/`'0'`), inline Form/View, empty → soft-sky `*StatisticsView`, QC/detail `DetailHeaderMenus` + `leading`) — rolled out for **Contacts, Invoices, Notes, Tasks, Requests, Teams, Matches, Garments** (lists + inventory), **Estimates, Files, Ingest, Cups, Slots, Clubdesk** (Guides + price list), **Mail providers, Pulse providers, AI Providers** (see CHANGELOG 2026-09-11 / 2026-09-12 / 2026-09-13 / 2026-09-15 / 2026-09-16). **Toolbar collapse** (header Menu button): one **global** preference `homebase.listToolbar.collapsed` (`'1'`/`'0'`) via `usePersistedToolbarCollapsed` / `listToolbarLayout.ts`; legacy per-plugin `*.toolbar.collapsed` keys migrate once (any-collapsed-wins) then are removed. **Filters visible** stays **per plugin** (`usePersistedFiltersVisible`, e.g. `homebase.contacts.toolbar.filtersVisible`). **Tasks** default sort: Created newest-first. **Garments** list soft-preview hydrates via `getList` for PersonMatrix. **Tasks / Requests / Notes:** toolbar `RoundExpandableQuickAdd` left of search.
...
| Element              | Standard                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Sidebar**          | Permanent rail from **`lg`** (desktop): expanded `252px` or collapsed `72px` icon rail (`LeftSidebarContext`, preference in localStorage `homebase.leftSidebar.collapsed`). `SidebarBrand` (logo + org name when expanded; logo-only when collapsed). **Collapsed rail:** category icons open `CollapsedCategoryFlyout` — hover (150ms) or click → Radix `DropdownMenu` (`side=right`, Sort-dropdown chrome); selecting a plugin navigates **without** expanding the sidebar. Submenu parents (e.g. Clubdesk, Garments) render as label + leaf items only. Edge `RoundIconLabelButton` (`xs`, `ChevronLeft`/`ChevronRight`, white / primary hover, `top-1`, `expandOnHover={false}`) is the **sole** expand/collapse control — category click does not expand. Phone and pad use overlay Sheet with full expanded nav (`collapsed={false}`; unchanged). Brand also appears at the top of the sheet. |
| **Right sidebar**    | Desktop only (`AppRightSidebar`). Fixed ~48px rail with round tools: User / Pomodoro / Timer open ~320px slide-outs; Settings goes to `/settings`; enabled companion-capable plugins (e.g. **Schedule**, **Garments inventory**) open a wider ~640px companion flyout globally. Rail toggles hide per plugin **`companionHideOnPrimaryPages`** (fallback: plugin `name` as primary); open flyout closes on the same surfaces (`shouldCloseCompanionForPrimary`). Phone/pad: rail hidden; account menu via floating `MobileUserMenu`. Plugin detail sidebars stay inline.                                                                                                                                                                                                                                                                                                                            |
...
| **Badges**            | Inline labels via `BADGE_CHIP_CLASS` / `StatusOutlineBadge`: icon + extrabold colored text only — **no** fill, border, fixed height, or padding. Colors: `QC_STATUS_BADGE_COLORS` / `DUE_DATE_BADGE_COLORS`. Compact: `BADGE_CHIP_COMPACT_CLASS`. Select triggers: `BADGE_SELECT_TRIGGER_CLASS`. |
...
- **Notes focus mode:** `NoteForm` (edit) only — Focus toggle dims the rest of the view (`bg-slate-950/55`), portals the note editor into a **viewport-centered** dialog (`max-w-[1080px]`, vertically/horizontally centered), hides secondary sections, Esc or click outside exits. Hint: `notes.focusModeHint`. **`NoteView` (detail) has no focus mode** (mail-layout tabs). Contacts full-view focus trial (2026-09-08) was **removed 2026-09-09** — do not reintroduce without a new design decision.
...
- **Schedule lock toggle:** `ScheduleLockToggle` (`client/src/plugins/schedule/components/ScheduleLockToggle.tsx`) — always-visible clickable icon beside schedule title: red `Lock` when locked, green `Unlock` when open; toggles via `setLockedForSchedule`. Used in `ScheduleList`, `PlanView`, and settings titles. No custom `iconClassName` prop (removed 2026-09-22; standard round-button icon size).
...
- Desktop only (`hidden lg:block`). Fixed ~48px rail — **does not expand**. Slide-out flyout (~320px) opens from the rail’s inner edge when a tool is active (`RightSidebarFlyout`: transform/opacity transition both open and close; title `text-lg`). Plugin **companion** Lists use the same flyout chrome at ~640px (`RIGHT_SIDEBAR_COMPANION_FLYOUT_WIDTH_PX`) and overlay the primary plugin — primary keeps full width (no MainLayout split). **Multi-companion (2026-09-22):** each registry entry with non-empty `canOpenAsCompanionFor` + tenant enablement may expose a rail toggle; implementations today: **Schedule** (`ScheduleList`, `isCompanion`) and **Garments** (`GarmentList`, inventory-only **view-only** via `isCompanion` — browse/soft-preview inside the flyout; no create/bulk in companion; optional **Open full item** closes companion and opens that article on `/garments/inventory`).
- Rail tools (top → bottom): User, Theme, Settings, then a gap, then Pomodoro / Timer widgets, then companion toggles for enabled companion-capable plugins (filtered with `shouldHideCompanionRailForPrimary` / `companionHideOnPrimaryPages`). User / Pomodoro / Timer toggle widget flyouts; Settings navigates to `/settings` (via `navigateToSettings`); Theme toggles light/dark; companion toggles the wider companion flyout (`CompanionPanelContext`). Widget and companion flyouts are mutually exclusive.
...
- **Companion flyout:** desktop-only; plugin **List** with `isCompanion` (Schedule calendar; Garments **inventory table only**, browse/view-only with optional **Open full item** escape). **Stays open across primary page changes** (`sessionStorage` key `homebase.companionPlugin`, allowlisted via `isRegisteredCompanionPlugin`); opens only when tenant-enabled (`isCompanionEnabled` / `getCompanionCandidates`); closes when primary matches that plugin’s hide surfaces (`companionHideOnPrimaryPages`, helpers in `companionPrimarySurface.ts` — e.g. Schedule on `/schedule`, Garments on `/garments/inventory`), leaving desktop, plugin disabled, or user dismiss. **Registry (companion chrome):** `canOpenAsCompanionFor` (discovery; host entries reserved/ignored); `companionHideOnPrimaryPages` (rail hide + auto-close); `companionRailTitleNavPage` (flyout/rail label → `nav.{page}`, default plugin `name`); `companionRailIcon` (rail button; default `navigation.icon`). Primary plugin keeps full width (no `MainLayout` split; legacy side-by-side `CompanionPanel` removed).
...
**Detail Form = Detail View chrome:** Plugin create/edit forms that use `DetailLayout` must match view mode: no extra outer padding / `md:-mx-6` bleed shell, no `PANEL_MAX_WIDTH` on the main column, cards use `DETAIL_VIEW_CARD_CLASS`. Content sits in DetailPanel’s outer inset (`px-2 sm:px-3` phone, `px-6` pad/desktop — same as list pages) with `pb-4` (same as view). **Fact fields** in Contacts-class detail edit use **ghost** tokens (`FORM_GHOST_*` in `formFieldStyles.ts`) with soft light-blue idle `bg-primary/10` so edit is visible while typography matches view (`DETAIL_FIELD_VALUE_CLASS`); hero titles use `DETAIL_FORM_TITLE_INPUT_CLASS`; multi-line prose uses `FORM_GHOST_TEXTAREA_CLASS` (`text-sm font-normal`, grows with content). **Filled** tokens remain for settings and dense grids. **Create/edit `?tab=` chips** (parity with View; Linked/Activity or Activity greyed in edit) apply to Contacts, Notes, Tasks, and Requests. **Leave confirm** while those forms are open (list / plugin settings / left sidebar / right-rail Settings / Close) uses the global discard dialog — session-based (`registerUnsavedChangesChecker(..., () => true)`), not dirty-only. See `PLUGIN_VIEW_IMPLEMENTATION_GUIDE.md` §3 and §5.3.
...
- **Quick context:** Mail-layout `*QuickContextPanel` or `*DetailHeaderMenus` with `leading` in the view header (Contacts / Cups / Slots). Do not duplicate ghost icon buttons. Legacy sticky list-side QC is removed (do not reintroduce).

...
| Share         | Tasks-style: Export → Share creates (or reuses) a 30-day link and opens `ShareDialog` — no valid-until picker. **Export → Email invoice** (mail plugin) ensures the same share link, then opens `BulkEmailDialog` with the customer as recipient and the public URL attached to the message body. Active share panel + public share page include **Download PDF** (`GET /api/invoices/public/:token/pdf`).                                                                                                                                                         |

**Security (2026-09-23, Email invoice):** Approved. Reuses authenticated share create + mail send (CSRF, plugin gates, high-entropy token). Ensuring the share before the compose dialog opens is the same residual class as Export → Share (cancel without Send leaves an active link; revoke remains available). No new TPM-accepted residual for this slice.

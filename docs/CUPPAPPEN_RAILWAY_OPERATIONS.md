...
| `/sok/`, `/kommande/`, `/alla/`, `/info/`, `/distrikt/` | SPA                      | Listing-tabs (ej hash); `/distrikt/` = översikt |
| `/{distrikt}/` (t.ex. `/skane/`)                        | SPA                      | Distriktsvy; ett path-segment (ej reserved)     |
| `/{distrikt}/{slug}-{år}`                               | `cup.php` (SSR)          | Kanonisk cup-detalj                             |
| `/cup/...`                                              | `cup.php` → **301**      | Legacy → kanonisk distrikts-URL                 |
| `/favicon.ico`                                          | **301** → `/favicon.svg` | Undvik SPA-HTML som ikon                        |

...
10. **Guide / price-list category ownership (form)** — Catalog CRUD lives on the edit form (**Guide category** / **Item categories** cards), not a plugin Settings page. **Superseded 2026-09-22:** Clubdesk has **no** settings shell (`ClubdeskSettingsView` and orphan `openClubdeskSettings` / `clubdeskContentView` / `ClubdeskSettingsTab` removed). Site Info is the dedicated nav page `/clubdesk/info` (`ClubdeskInfoView`). Assigning a guide’s `category`: click the category name on the Guide category card (click again → uncategorized). Information card has no category `Select`.
...
**Price list ↔ inventory (Epic 2, 2026-09-25):** Optional FKs on price-list items + dual price (`price_override` / live catalog, migration **173**); public `inventorySlug` when inventory published — [`CLUBDESK_INVENTORY_EPIC2.md`](CLUBDESK_INVENTORY_EPIC2.md).
...
| Item    | Value                                                                                                                                                                                                                                                                                                                                          |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Migrate | `npm run migrate:clubdesk` (tenant: 119, 120, **122**, **123**, **171** inventory, **172** price-list↔inventory, **173** price_override; main grant: 121) or `npm run migrate:clubdesk-inventory` (**171**) / `npm run migrate:clubdesk-price-list-inventory-link` (**172**) / `npm run migrate:clubdesk-price-list-price-override` (**173**) |

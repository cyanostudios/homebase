...
`ContactView` main column / Linked tiles include related plugin rows when the plugin is enabled. Sidebar/nav-oriented related cards historically followed Notes → Tasks → Teams → Matches → Slots → Estimates; **Linked tiles** in `ContactLinkedItemsSection` also include Garments and Invoices when enabled:
...
| Invoices  | `invoices`  | `invoicesApi.getItems()` filtered by `contactId` (client)                                         | Name → `AssignmentQuickInfoDialog`; Open → `openInvoiceForView`. **Create:** Actions → Invoice via `requestInvoiceCreateFromContact` (not `useInvoices` in `ContactProvider`) |
...
Quick context Linked preview: max **2** tiles then “X more linked items” (`previewLimit={2}`).

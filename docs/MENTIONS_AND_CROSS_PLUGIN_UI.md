...
| `TeamMatchesSection`                       | Match        | Name → `MatchQuickInfoDialog`; Open → `openMatchForView` (cross-plugin navigate). Tab UI: home/away groups vs upcoming-by-date chips; side classification uses Matches settings `defaultHomeTeam` (see product changelog 2026-08-09). |

...
**Form data fields:** Contacts-class **detail** create/edit fact fields use shared **ghost** tokens (`FORM_GHOST_*` in `formFieldStyles.ts`) with soft light-blue idle `bg-primary/10` so edit is visible and typography matches view values. Multi-line prose uses `FORM_GHOST_TEXTAREA_CLASS` (`text-sm font-normal`; height sync via `syncTextareaHeight` where wired). **Create/edit `?tab=`** (same chips as View; Linked/Activity or Activity disabled in edit) is required for Contacts-class plugins that have View tabs (incl. inventory). Where Information and Properties were separate chips, merge into one **Information** tab with Properties as card #2 (legacy `?tab=properties` → `information`; Estimates keeps Properties as primary facts tab). Aligned mail-layout forms register session leave `() => true` while open so leave via list / settings / sidebars / Close confirms discard (not dirty-only). Plugin **settings** and dense in-plugin editors (e.g. garments `PersonMatrix` / inventory QC quantity) use filled `FORM_*` / `FORM_COMPACT_*`. Do **not** invent ad-hoc `h-9`/`h-10` bordered inputs on plugin forms. Dialogs and list search keep default shadcn chrome unless a documented dense-dialog exception applies (see `PLUGIN_VIEW_IMPLEMENTATION_GUIDE.md` §3).

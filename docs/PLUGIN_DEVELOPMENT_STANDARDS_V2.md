...
- **List layout:** **Table-only** via `*ListTable` (`SortableListTable`, `rowBorders={false}`) + `ListFooterBar`. No `ListColumnLayoutToggle` / cards grid. Visible columns are **code defaults** (name/title only) — do **not** add settings **Table columns** prefs (`PLUGIN_VIEW_IMPLEMENTATION_GUIDE.md` §4). Golden template: `templates/plugin-frontend-template`.

...
interface MyPluginContextType {
  ...
  panelMode: 'create' | 'edit' | 'view'; // Preferred. Settings use *ContentView === 'settings' + *SettingsView — do not add 'settings' to panelMode
...
Use the same UI components and styling as other plugins so list views and toolbars look and behave the same across the app. **New plugins** start from `templates/plugin-frontend-template` (**table-only** list shell). Canonical rules: [UI_AND_UX_STANDARDS_V3.md](UI_AND_UX_STANDARDS_V3.md) §0.1.

- **Toolbar:** Shared `ListToolbar` (`@/core/ui/ListToolbar`) where still used: idle = Select all + optional `beforeSearch` + `ListSearchInput` (clear X) + sort; selection = bulk actions replace that row. Canonical CRUD lists use the **page header** pattern (Select/Clear + `BulkActionRoundBar` + search + Add) — **table-only**, no cards/column layout toggle. Do **not** use legacy `ContentToolbar` header injection, and do **not** use an in-card grid/list toggle (`viewMode`).
...
- **List layout:** **Table-only** via `*ListTable` (`SortableListTable`, `rowBorders={false}`) + `ListFooterBar`. No `ListColumnLayoutToggle` / cards grid. Default row = **title + meta** on one identity column (name/title only) — do **not** add settings **Table columns** prefs or invent multi-column lists as default (`PLUGIN_VIEW_IMPLEMENTATION_GUIDE.md` §4). Golden template: `templates/plugin-frontend-template`.
...
> **Aug 2026:** Full-page plugin settings use `PluginSettingsPageShell` + `SettingsCategoryCard` so layout matches Core Settings (title/subtitle, category cards, header Save when dirty, Close in trailing). See `UI_AND_UX_STANDARDS_V3.md` §3.2. Legacy panel settings still apply only where a plugin has not migrated (see below).

If the plugin exposes a settings screen (e.g. cloud storage, SMTP, preferences), implement one of:

- **Full-page (preferred):** `*SettingsView` on the list route via `*ContentView === 'settings'`. Wrap content in `PluginSettingsPageShell` (`@/core/ui/PluginSettingsPageShell`). Pass Close as `trailing` / `inlineTrailing`. Use category cards (`SettingsCategoryCard`) when there are ≥2 categories; put dirty Save in `saveAction` (header), not a bottom footer. See `UI_AND_UX_STANDARDS_V3.md` §3.2 for save variants (dirty header vs per-section). Do **not** add unused `renderCategoryButtonsInline` (removed from Estimates/Invoices/Cups/Matches settings and Mail/Pulses/AI Providers routing shells 2026-09-22; prop was never read). **Verified full-page only (no `panelMode === 'settings'` / no `*SettingsForm` in the detail `Form`):** Contacts (`ContactSettingsView`), Notes (`NotesSettingsView`), Tasks (`TaskSettingsView`), Estimates (`EstimateSettingsView`), Invoices (`InvoiceSettingsView`), Cups (`CupsSettingsView`), Teams (`TeamsSettingsView`), Matches (`MatchSettingsView`), Schedule (`ScheduleSettingsView`), Files (`FileSettingsView` — body is still `FileSettingsForm`, mounted only from the settings view, not from `FileForm`). **Clubdesk:** no plugin settings shell (category catalog on form; Info via `/clubdesk/info`).
- **Panel (legacy — do not use for new plugins):** Extend `panelMode` to include `'settings'` (e.g. `'create' | 'edit' | 'view' | 'settings'`). Expose `openMyPluginSettings` (or `openMyPluginPanel` with a dedicated entry point) that sets `panelMode` to `'settings'` and opens the panel. Expose `closeMyPluginPanel` so the panel can be closed from shared panel actions (header/footer). **As of 2026-09-22 no client plugin Form still branches on `panelMode === 'settings'`** (Files dual path removed); keep this section only as a migration anti-pattern for new work.

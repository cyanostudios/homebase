...
- **List layout:** **Table-only** via `*ListTable` (`SortableListTable`, `rowBorders={false}`) + `ListFooterBar`. No `ListColumnLayoutToggle` / cards grid. Optional visible-column prefs in settings. Golden template: `templates/plugin-frontend-template`.

...
- **Quick context:** implement a full-only `*QuickContextPanel` as the first card of `*View` (copy `ContactQuickContextPanel` / view guide §1). Do **not** add `useQuickContextPreview` list-side QC unless product requires the Slots pattern. No delete/duplicate in the QC body.

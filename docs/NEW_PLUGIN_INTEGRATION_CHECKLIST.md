...
- **Full-page settings:** Use `PluginSettingsPageShell` + optional `SettingsCategoryCard` (see `UI_AND_UX_STANDARDS_V3.md` §3.2). Examples: `NotesSettingsView`, `TaskSettingsView`, `TeamsSettingsView`, `MatchSettingsView` via `*ContentView === 'settings'` (not panel `Form`; do not add `panelMode: 'settings'`).

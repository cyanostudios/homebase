...
## Inspektera i databasen

Med aktivt tenant-schema (t.ex. `SET search_path TO tenant_1, public`):

```sql
SELECT m.*, ci.channel, ci.instance_key
FROM channel_product_map m
LEFT JOIN channel_instances ci ON ci.id = m.channel_instance_id
WHERE m.product_id::text = '<productId>';
```

Byt ut `<productId>` mot önskat id.

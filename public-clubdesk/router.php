<?php
declare(strict_types=1);

$uriPath = parse_url($_SERVER['REQUEST_URI'] ?? '/', PHP_URL_PATH) ?: '/';
$docRoot = rtrim($_SERVER['DOCUMENT_ROOT'] ?? __DIR__, '/');
$targetPath = realpath($docRoot . $uriPath);

// Browsers request /favicon.ico; prefer PNG so tabs show the mark reliably.
if ($uriPath === '/favicon.ico') {
    header('Location: /icons/favicon-32.png', true, 301);
    return true;
}

// Let built-in server serve existing static files directly (no PHP processing).
if ($uriPath !== '/' && $targetPath !== false && str_starts_with($targetPath, $docRoot) && is_file($targetPath)) {
    return false;
}

if ($uriPath === '/sitemap.xml') {
    require __DIR__ . '/api/sitemap.php';
    return true;
}

if ($uriPath === '/guide' || str_starts_with($uriPath, '/guide/')) {
    require __DIR__ . '/guide.php';
    return true;
}

if ($uriPath === '/price-list' || str_starts_with($uriPath, '/price-list/')) {
    require __DIR__ . '/price-list.php';
    return true;
}

if (preg_match('#^/inventory/([a-z0-9-]+)/?$#i', $uriPath) === 1) {
    require __DIR__ . '/inventory.php';
    return true;
}

if ($uriPath === '/swish' || $uriPath === '/swish/') {
    require __DIR__ . '/swish.php';
    return true;
}

if ($uriPath === '/kontakt' || $uriPath === '/kontakt/') {
    require __DIR__ . '/kontakt.php';
    return true;
}

// SPA listing paths — serve index.html (real URLs, not hash).
if (
    preg_match('#^/(guides|alla|price-lists|inventory|info)/?$#', $uriPath) === 1
    || preg_match('#^/kategori/[^/]+/?$#', $uriPath) === 1
) {
    require_once __DIR__ . '/api/security_headers.php';
    applyPublicAppSecurityHeaders('html');
    readfile(__DIR__ . '/index.html');
    return true;
}

require_once __DIR__ . '/api/security_headers.php';
applyPublicAppSecurityHeaders('html');
readfile(__DIR__ . '/index.html');
return true;

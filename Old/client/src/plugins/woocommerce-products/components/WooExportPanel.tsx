...
export const WooExportPanel: React.FC = () => {
  ...
  const handleExport = async () => {
    ...
    const ok = await exportProducts(mvpPayload as any);
    // Auto-scroll till resultat så användaren ser feedback direkt
    if (ok) {
      setTimeout(() => {
        const el = document.getElementById('woo-export-result');
      ...
      {isConfigured && (
        <Card padding="lg">
          <div className="flex flex-col gap-3">
            ...
            {/* Export result */}
            {lastExportResult && (
              <div id="woo-export-result" className="mt-3 rounded-xl border p-3 text-sm">
                <div className="flex items-center justify-between">
                  <div

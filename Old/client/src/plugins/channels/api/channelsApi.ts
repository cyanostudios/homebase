...
getProductMap(params: { productId: string; channel: string }) {
  const q = new URLSearchParams({ productId: params.productId, channel: params.channel });
  return this.request(`/map?${q.toString()}`) as Promise<{ ok: true; row: ChannelMapRow | null }>;
}
...
// Enskild rad i channel_product_map (för läsning av status per produkt/kanal)

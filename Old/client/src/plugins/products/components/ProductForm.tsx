...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  // lokal fältvalidering för SKU (kompletterar server- och context-validering)
  ...
  const updateField = (field: keyof FormData, value: string | number | string[]) => {
    ...
    if (field === 'sku') {
      ...
      setLocalSkuError(v ? null : 'SKU is required');
    }
    markDirty();
  ...
  const handleSubmit = useCallback(async () => {
    // Guard: SKU måste finnas
    if (!String(formData.sku || '').trim()) {
      setLocalSkuError('SKU is required');
      // scrolla till SKU-fältet (lätt UX-förbättring)
      const el = document.getElementById('product-form-sku');
  ...
  const skuFieldError = getFieldError('sku')?.message || localSkuError;
          ...
          <Card padding="sm" className="shadow-none px-0">
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              ...
              <ul className="mt-2 list-disc list-inside text-sm text-red-700">
                ...
                {/* Om SKU saknas men inte finns i context-validering, visa här också */}
                {!getFieldError('sku') && localSkuError && <li>{localSkuError}</li>}
              </ul>
            </div>
          </Card>
        )}
          ...
          <div className="space-y-3 md:space-y-0 md:grid md:grid-cols-3 md:gap-3">
            ...
            <div>
              ...
              <input
                ...
                required
                className={`w-full px-3 py-1.5 text-base border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                  skuFieldError ? 'border-red-500' : 'border-gray-300'
                }`}
              />
              {skuFieldError && (
                <p className="mt-1 text-sm text-red-600">{skuFieldError}</p>
              )}
            </div>
              ...
              {getFieldError('gtin') && (
                <p
                  className={`mt-1 text-sm ${
                    getFieldError('gtin')?.message.includes('Warning')
                      ? 'text-yellow-600'
                      : 'text-red-600'
                  }`}
                >

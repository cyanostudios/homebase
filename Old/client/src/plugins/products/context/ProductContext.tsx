...
interface ProductContextType {
  ...
  deleteProducts: (ids: string[]) => Promise<void>; // NEW bulk
...
export function ProductProvider({
  ...
  const validateProduct = (data: any): ValidationError[] => {
    ...
    if (data.productNumber?.trim()) {
      ...
      if (clash) {
        add(
          'productNumber',
          `Product number "${pn}" already exists (used by "${
            clash.title ?? clash.companyName ?? 'another product'
          }")`,
        );
      }
    }
  ...
  const deleteProducts = async (ids: string[]) => {
    ...
    try {
      // Primärt: riktig bulk-endpoint
      if (typeof (productsApi as any).deleteProductsBulk === 'function') {
        await (productsApi as any).deleteProductsBulk(uniqueIds);
      } else {
        // Fallback: parallella enskilda anrop
        await Promise.all(uniqueIds.map((id) => productsApi.deleteProduct(id)));
      }
